...
* [VM](https://visualvm.github.io/)visual: Visual VM le permite ver información detallada de la VM de Java, incluso el uso de la CPU, el uso de la memoria Java. Además, le permite tomar muestras y evaluar el código que se ejecuta en una instancia.
* [Superior](http://man7.org/linux/man-pages/man1/top.1.html): Arriba hay un comando de Linux que abre un tablero, que muestra estadísticas de uso, incluyendo el uso de CPU, memoria y E/S. Proporciona información general de alto nivel sobre lo que sucede en una instancia.
...
* [Iotop](http://guichaz.free.fr/iotop/): Iotop es un tablero detallado para el uso de E/S de disco. Muestra barras y medidores que representan los procesos que utilizan la E/S de disco y la cantidad que utilizan. Iotop puede instalarse en la mayoría de los sistemas Linux usando `yum install iotop` o `apt-get install iotop`.

* [Ftop](http://www.ex-parrot.com/pdw/iftop/): Iftop muestra información detallada sobre el uso de Ethernet/red. Iftop muestra las estadísticas por canal de comunicación en las entidades que utilizan Ethernet y la cantidad de ancho de banda que utilizan. Si se puede instalar en la mayoría de los sistemas Linux usando `yum install iftop` o `apt-get install iftop`.
...
* [Consola](../sites-administering/workflows.md)de flujo de trabajo: Aproveche la consola de flujo de trabajo para supervisar los flujos de trabajo que se retrasan o se quedan atascados.
...
* Umbral de alarma: Cuando el valor de `QueueBlocked` es true o el valor de `QueueNumEntries` es superior al 150% de la línea base.
...
* Actualice su servidor AEM. Es posible que no tenga un tamaño adecuado para la instancia de AEM. La asistencia técnica de Adobe puede ayudarle a identificar si el servidor no tiene el tamaño adecuado.
