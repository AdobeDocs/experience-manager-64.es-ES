...
description: Obtenga información sobre varias tareas de edición y administración de recursos que puede realizar mediante la interfaz de usuario táctil de Recursos AEM
contentOwner: AG
mini-toc-levels: 1
...
1. Para cargar los recursos, realice una de las siguientes acciones:
   ...
   * En un navegador compatible con HTML5, arrastre los recursos directamente en la interfaz. No se muestra el cuadro de diálogo para cambiar el nombre del archivo.
   ...
   Puede pausar la carga de recursos de gran tamaño (más de 500 MB) y reanudarla más tarde desde la misma página. Toque el icono **[!UICONTROL Pausa]** junto a la barra de progreso que aparece cuando se inicia la carga.

   ![climage_1-5](assets/chlimage_1-5.png)

   Se puede configurar el tamaño por encima del cual un recurso se considera un recurso grande. Por ejemplo, puede configurar el sistema para que considere los recursos superiores a 1000 MB (en lugar de 500 MB) como recursos grandes. En este caso, el botón **[!UICONTROL Pausa]** de la barra de progreso aparece cuando se cargan recursos de tamaño superior a 1000 MB.

   El botón **[!UICONTROL Pausar]]**no muestra si se carga un archivo mayor que 1000 MB con un archivo menor que 1000 MB. Sin embargo, si cancela la carga de archivos de menos de 1000 MB, aparece el botón**[!UICONTROL Pausa ]**.
...
1. Cerca de la esquina inferior derecha de la página, toque Opciones **[!UICONTROL de]** trabajo y, a continuación, defina las opciones que desee en función de los recursos de la carpeta seleccionada.
   ...
   >[!NOTE]
   >
   >Al cargar recursos por medio de FTP, las opciones de trabajo de carga definidas en Dynamic Media Classic (Scene7) prevalecen sobre los parámetros de procesamiento de recursos establecidos en AEM.
...
|  | Cantidad | Controla la cantidad de contraste que se aplica a los píxeles del borde.<br> Piensen en ello como la intensidad del efecto. La diferencia principal entre los valores de cantidad de la máscara de enfoque en los medios dinámicos y los valores de cantidad en Adobe Photoshop es que Photoshop tiene un rango de cantidad del 1% al 500%. Mientras que, en Dynamic Media, el rango de valores es de 0,0 a 5,0. Un valor de 5,0 es el equivalente aproximado del 500 % en Photoshop; un valor de 0,9 es el equivalente al 90 %, y así sucesivamente. |
...
|  | Método de relleno | Requerido.<br> Controla la transparencia de píxeles desde la ubicación Esquina que haya definido.<br> Puede elegir entre los siguientes métodos de relleno: <ul><li>**Relleno** de inundación: hace que todos los píxeles sean transparentes y coincidan con la esquina especificada y estén conectados a ella.</li><li>**Coincidir con píxeles** : hace que todos los píxeles coincidentes sean transparentes, independientemente de su ubicación en la imagen.</li></ul> |
|  | Tolerancia | Opcional.<br> Controla la cantidad de variación permitida en la coincidencia de color de píxeles según la ubicación de esquina que haya definido.<br> Utilice un valor de 0,0 para coincidir exactamente con los colores de píxeles o utilice un valor de 1,0 para permitir la mayor variación. |
...
1. Vaya a la ubicación en la que desea copiar los recursos.
   ...
   >[!NOTE]
   >
   >Si copia un recurso en la misma ubicación, AEM genera automáticamente una variación del nombre. Por ejemplo, si copia un recurso denominado Cuadrado, AEM genera automáticamente el título de su copia como Cuadrado1.
...
Esto se debe a que Recursos AEM espera hasta la expiración de un período de tiempo de espera (1 hora de forma predeterminada) antes de ejecutar un trabajo en segundo plano para indexar los metadatos de todos los recursos recién cargados o actualizados y agregarlos a la lista de sugerencias.
...
1. Descargue o imprima el PDF con las opciones de la parte superior derecha.
   ...
   Para modificar el aspecto del archivo PDF procesado, por ejemplo, el color de fuente, el tamaño y el estilo, el color de fondo de los comentarios y estados, abra la configuración **[!UICONTROL de]** anotación PDF desde **[!UICONTROL Configuration Manager]** y modifique las opciones deseadas. Por ejemplo, para cambiar el color de visualización del estado aprobado, modifique el código de color en el campo correspondiente. Para obtener información sobre cómo cambiar el color de fuente de las anotaciones, consulte [Anotación](managing-assets-touch-ui.md#annotating).

   ![chlimage_1-44](assets/chlimage_1-44.png)

   Vuelva al archivo PDF procesado y actualícelo. El PDF actualizado refleja los cambios realizados.

**Para imprimir anotaciones en idiomas** extranjeros:Si un recurso incluye anotaciones en idiomas extranjeros (especialmente en idiomas no latinos), primero debe configurar el servicio CQ-DAM-Handler-Gibson Font Manager en el servidor AEM para poder imprimir estas anotaciones. Al configurar el servicio CQ-DAM-Handler-Gibson Font Manager, proporcione la ruta donde se encuentran las fuentes para los idiomas deseados.
