---
title: Administrar los recursos digitales mediante  [!DNL Experience Manager] Assets
description: Obtenga información sobre las diversas tareas de edición y administración de recursos que puede realizar mediante la interfaz de usuario táctil de  [!DNL Experience Manager] Assets
contentOwner: AG
mini-toc-levels: 1
...
Obtenga información sobre las diversas tareas de edición y administración de recursos que puede realizar mediante la interfaz de usuario táctil de [!DNL Experience Manager] Assets.

En este artículo se describe cómo administrar y editar recursos mediante la interfaz de usuario táctil de Adobe Experience Manager Assets. Para obtener información básica sobre la interfaz de usuario, consulte [Gestión básica de la IU táctil](/help/sites-authoring/basic-handling.md). Para administrar los fragmentos de contenido, consulte [Administración de fragmentos de contenido](content-fragments-managing.md) recursos.

## Crear carpetas {#create-folders}

Al organizar una colección de recursos, por ejemplo, todas las imágenes `Nature`, puede crear carpetas para mantenerlas juntas. Puede utilizar carpetas para categorizar y organizar los recursos. [!DNL Experience Manager] Los recursos no requieren que organice los recursos en carpetas para que funcionen mejor.
...
1. Para cargar los recursos, siga uno de estos procedimientos:
   ...
   * En un explorador compatible con HTML5, arrastre los recursos directamente en la interfaz. No se muestra el cuadro de diálogo para cambiar el nombre del archivo.
   ...
   Para cancelar una carga continua, haga clic en el botón `X` situado junto a la barra de progreso. Al cancelar la operación de carga, [!DNL Experience Manager] Assets elimina la parte parcialmente cargada del recurso.
   ...
   * el nombre de la carpeta de activos no debe contener `* / : [ \ \ ] | # % { } ? \" . ^ ; + & \t`
   ...
   Si cancela la operación de carga antes de que se carguen los archivos, [!DNL Experience Manager] Assets dejará de cargar el archivo actual y actualizará el contenido. Sin embargo, los archivos que ya se han cargado no se eliminan.

### Cargas serie {#serial-uploads}

La carga masiva de numerosos recursos consume importantes recursos del sistema, lo que puede afectar negativamente al rendimiento de su implementación [!DNL Experience Manager]. Los posibles cuellos de botella pueden ser la conexión a Internet, las operaciones de lectura y escritura en disco, las limitaciones del explorador web en el número de solicitudes de POST al cargar recursos simultáneamente. La operación de carga masiva puede fallar o finalizar prematuramente. En otras palabras, los activos [!DNL Experience Manager] pueden perder algunos archivos al ingerir un conjunto de archivos o no pueden ingerir ningún archivo.

Para superar esta situación, [!DNL Experience Manager] Assets ingesta un recurso a la vez (carga serie) durante una operación de carga masiva, en lugar de la ingesta simultánea de todos los recursos.
...
|  | Incluir subcarpetas | Cargue todas las subcarpetas dentro de la carpeta que desee cargar. Los nombres de la carpeta y sus subcarpetas que cargue se introducen automáticamente en [!DNL Experience Manager] Assets. |
...
Si carga numerosos recursos, las llamadas de E/S al servidor [!DNL Experience Manager] aumentan drásticamente, lo que reduce la eficacia de la carga e incluso puede hacer que se agote el tiempo de espera. [!DNL Experience Manager] Recursos admite la carga de recursos por secuencias. La carga por secuencias reduce la E/S de disco durante la operación de carga al evitar el almacenamiento de recursos en una carpeta temporal del servidor antes de copiarlo en el repositorio. En su lugar, los datos se transfieren directamente al repositorio. De este modo, se reduce el tiempo de carga de recursos grandes y la posibilidad de tiempos de espera. La carga de flujo continuo está habilitada de forma predeterminada en [!DNL Experience Manager] Assets.

La carga de transmisión está deshabilitada para [!DNL Experience Manager] que se ejecuta en el servidor JEE con una versión de servlet-api inferior a 3.1.
...
1. En el panel **[!UICONTROL Representaciones]**, consulte la lista de representaciones generadas para el recurso.
   ...
   >[!NOTE]
   >
   >De forma predeterminada, [!DNL Experience Manager] Assets no muestra la representación original del recurso en el modo de vista previa. Si es administrador, puede utilizar superposiciones para configurar [!DNL Experience Manager] Assets de modo que muestre las representaciones originales en el modo de vista previa.
...
Esto se debe a que [!DNL Experience Manager] Assets espera hasta la caducidad de un tiempo de espera (1 hora de forma predeterminada) antes de ejecutar un trabajo en segundo plano para indexar los metadatos de todos los recursos recién cargados/actualizados y agregarlos a la lista de sugerencias.
...
1. Para ver la notificación, inicie sesión en [!DNL Experience Manager] Assets with Aaron MacDonald&#39;s credentials y pulse el icono **[!UICONTROL Notifications]** para ver la notificación.
...
1. Reinicie la instancia [!DNL Experience Manager].
...
* Modifique una imagen en otra aplicación y cárguela en [!DNL Experience Manager] Assets. Se crea una versión de la imagen para que la imagen original no se sobrescriba.
...
Consulte [aplicar un flujo de trabajo a un  [!DNL Experience Manager] recurso](/help/assets/assets-workflow.md#apply-a-workflow-to-an-aem-asset).
