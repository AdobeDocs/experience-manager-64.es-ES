...
>[!NOTE]
>
>Consulte la página [Formatos compatibles con Assets](assets-formats.md) para obtener una descripción de todos los formatos compatibles con Recursos Experience Manager y las funciones compatibles con cada formato.

## Controladores de medios predeterminados {#default-media-handlers}

Los siguientes controladores de medios están disponibles en Recursos de Experience Manager y administran los tipos MIME más comunes:
...
1. Haga clic en el enlace `com.day.cq.dam.core.impl.store.AssetStoreImpl`.
...
El Experience Manager tiene algunos flujos de trabajo predeterminados para procesar los recursos. Para verlos, abra la consola Flujo de trabajo y haga clic en la pestaña **[!UICONTROL Modelos]**: los títulos de flujo de trabajo que comienzan con Recursos de Experience Manager son los específicos de los recursos.
...
El siguiente ejemplo muestra cómo mejorar el flujo de trabajo **[!UICONTROL AEM Assets Synchronization]** para que se generen subrecursos para todos los recursos, excepto para los documentos PDF.
...
1. En Eclipse, cree el proyecto `myBundle` Maven:

   1. En la barra de menús, haga clic en **[!UICONTROL Archivo > Nuevo > Otro]**.
   1. En el cuadro de diálogo, expanda la carpeta Maven, seleccione Maven Project y haga clic en **[!UICONTROL Next]**.
   1. Marque la casilla **[!UICONTROL Create a simple project]** y la casilla **[!UICONTROL Use default Workspace locations]** y haga clic en **[!UICONTROL Next]**.
...
1. Establezca el compilador de Java™ en la versión 1.5:
   ...
   1. Haga clic con el botón derecho en el proyecto `myBundle` y seleccione Propiedades.
...
1. Cree el paquete `com.day.cq5.myhandler` que contiene las clases Java™ en `myBundle/src/main/java`:
...
1. Cree la clase Java™ `MyHandler`:
   ...
   1. En Eclipse, en `myBundle/src/main/java`, haga clic con el botón derecho en el paquete `com.day.cq5.myhandler`, seleccione Nuevo y, a continuación, Clase.
...
1. Compile la clase Java™ y cree el paquete:
   ...
   1. Haga clic con el botón derecho en el proyecto myBundle, seleccione **[!UICONTROL Ejecutar como]** y, a continuación, **[!UICONTROL Maven Install]**.
...
1. Copie el paquete `myBundle-0.0.1-SNAPSHOT.jar` y guárdelo en `/apps/myApp/install` (por ejemplo, con WebDAV). El nuevo controlador de texto está ahora activo en Experience Manager.
1. En el navegador, abra la Consola de Gestión Web Apache Felix. Seleccione la pestaña Componentes y deshabilite el controlador de texto predeterminado `com.day.cq.dam.core.impl.handler.TextHandler`.
...
El siguiente ejemplo muestra cómo configurar el paso de proceso de la línea de comandos. Cada vez que se añade un recurso con el tipo MIME gif o tiff a `/content/dam` en el servidor Experience Manager, se crea una imagen giratoria del original junto con tres miniaturas más (140x100, 48x48 y 10x250).
...
1. Para ver si la herramienta se ejecuta correctamente, añada una imagen JPG al directorio de trabajo y ejecute el comando `convert <image-name>.jpg -flip <image-name>-flipped.jpg` en la línea de comandos.
...
1. Cambie la configuración del paso **[!UICONTROL Web enabled rendition]** de la siguiente manera:
...
1. Abra el recurso `myImage.tiff` y compruebe que se hayan creado la imagen volteada y las tres miniaturas.

#### Configurar el paso del proceso CommandLineProcess {#configuring-the-commandlineprocess-process-step}

En esta sección se describe cómo establecer los **[!UICONTROL argumentos de proceso]** de `CommandLineProcess`. Separe los valores de [!UICONTROL Process Arguments] con una coma y no inicie un valor con un espacio en blanco.
...
| cmd: &lt;command> | Define el comando que se ejecuta. La sintaxis depende de la herramienta de línea de comandos. Solo se puede definir un comando. <br>Para crear el comando se pueden usar las siguientes variables: <br>`${filename}`: nombre del archivo de entrada, por ejemplo original.jpg  <br> `${file}`: nombre de ruta completo del archivo de entrada, por ejemplo /tmp/cqdam0816.tmp/original.jpg  <br> `${directory}`: del archivo de entrada, por ejemplo /tmp/cqdam0816.tmp  <br>`${basename}`: nombre del archivo de entrada sin su extensión, por ejemplo original  <br>`${extension}`: extensión del archivo de entrada, por ejemplo jpg |

Por ejemplo, si ImageMagick está instalado en el disco que aloja el servidor Experience Manager y si crea un paso de proceso utilizando **CommandLineProcess** como Implementación y los siguientes valores como **Process Arguments**:

`mime:image/gif,mime:image/tiff,tn:140:100,tn:48:48,tn:10:250,cmd:convert ${directory}/${filename} -flip ${directory}/${basename}.flipped.jpg`

A continuación, cuando se ejecute el flujo de trabajo, el paso solo se aplicará a los recursos que tengan `image/gif` o `mime:image/tiff` como tipos de mime. Crea una imagen girada del original, la convierte en .jpg y crea tres miniaturas con las dimensiones: 140x100, 48x48 y 10x250.

Utilice los siguientes [!UICONTROL Argumentos de proceso] para crear las tres miniaturas estándar con ImageMagick:

`mime:image/tiff,mime:image/png,mime:image/bmp,mime:image/gif,mime:image/jpeg,cmd:convert ${filename} -define jpeg:size=319x319 -thumbnail "319x319>" -background transparent -gravity center -extent 319x319 -write png:cq5dam.thumbnail.319.319.png -thumbnail "140x100>" -background transparent -gravity center -extent 140x100 -write cq5dam.thumbnail.140.100.png -thumbnail "48x48>" -background transparent -gravity center -extent 48x48 cq5dam.thumbnail.48.48.png`

Utilice los siguientes [!UICONTROL Argumentos de proceso] para crear la representación habilitada para web mediante ImageMagick:
