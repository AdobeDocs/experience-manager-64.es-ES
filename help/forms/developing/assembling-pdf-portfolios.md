...
Se debe hacer referencia a un documento DDX para montar un Portfolio PDF. Este documento DDX debe contener los `Portfolio`elementos `Navigator` y `PackageFiles` .
...
Para montar un Portfolio PDF, debe llamar a la `invokeDDX` operación. El servicio Ensamblador devuelve el Portfolio PDF dentro de un objeto de colección.
...
1. Monte el portafolio.
   ...
   * Un `java.util.Map` objeto que contiene los archivos necesarios para crear un Portfolio PDF.
   * Un `com.adobe.livecycle.assembler.client.AssemblerOptionSpec` objeto que especifica las opciones de tiempo de ejecución, incluyendo la fuente predeterminada y el nivel de registro de trabajos
   ...
   El `invokeDDX` método devuelve un `com.adobe.livecycle.assembler.client.AssemblerResult` objeto que contiene el Portfolio PDF ensamblado y las excepciones que se hayan producido.
...
1. Guarde el portafolio montado.
   ...
   * Invocar el `com.adobe.idp.Document` método del `copyToFile` objeto para extraer el Portfolio PDF.
...
1. Haga referencia a los documentos necesarios.
   ...
   * Create a `MyMapOf_xsd_string_To_xsd_anyType` object. Este objeto de colección se utiliza para almacenar los archivos de entrada necesarios para crear un Portfolio PDF.
