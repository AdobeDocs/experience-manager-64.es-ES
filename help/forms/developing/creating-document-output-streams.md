...
1. Recupere los resultados de la operación.
   ...
   * Invoque el `com.adobe.idp.Document` método del `copyToFile` objeto para copiar el contenido del `com.adobe.idp.Document` objeto en el archivo (asegúrese de utilizar el `com.adobe.idp.Document` objeto devuelto por el `getStatusDoc` método).
   >[!NOTE]
   >
   >Aunque el servicio Output escribe el documento PDF/A en la ubicación especificada por el argumento que se pasa al `PDFOutputOptionsSpec` método del `setFileURI` objeto, puede recuperar mediante programación el documento PDF/A invocando el método `OutputResult` * del `getGeneratedDoc`objeto.*
