...
description: Utilice la API de Java del servicio de administración de Documento para crear espacios de Content Services, eliminar espacios de Content Services, Añadir contenido a Content Services, recuperar contenido de Content Services, mover contenido de Content Services, lista de contenido de Content Services, buscar contenido de Content Services y establecer permisos de Content Services.
...
El siguiente ejemplo de código Java crea un nuevo espacio llamado *Test Directory *ubicado en el Inicio de Compañía. El valor de identificación del nuevo espacio se escribe en la consola.
...
## Inicio rápido (modo SOAP): Configuración de los permisos de Content Services mediante la API de Java (obsoleto) {#quick-start-soap-mode-setting-content-services-permissions-using-the-java-api-deprecated}

El siguiente ejemplo de código Java establece un permiso para un usuario con el nombre tony blue. El dominio especificado es el dominio predeterminado. Se especifica el permiso de consumidor y el nodo es `/Company Home/Test Directory`.
