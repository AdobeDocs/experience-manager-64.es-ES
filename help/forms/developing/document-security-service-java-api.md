...
description: 'Utilice la API de Java del servicio de administración de derechos para crear una política, modificar una política, eliminar una política, aplicar una política a un documento PDF, quitar una política de un documento PDF, revocar un documento, restablecer el acceso a un documento revocado, inspeccionar documentos PDF protegidos por políticas, crear una marca de agua, modificar una marca de agua, buscar eventos y eliminar una política de un documento de Word. '
...
[Inicio rápido (modo SOAP): Modificación de una política mediante la API de Java](#quick-start-soap-mode-modifying-a-policy-using-the-java-api)

[Inicio rápido (modo SOAP): Eliminación de una directiva mediante la API de Java](document-security-service-java-api.md#quick-start-soap-mode-deleting-a-policy-using-the-java-api)

[Inicio rápido (modo SOAP): Aplicación de una política a un documento PDF mediante la API de Java](#quick-start-soap-mode-applying-a-policy-to-a-pdf-document-using-the-java-api)

[Inicio rápido (modo SOAP): Eliminación de una política de un documento PDF mediante la API de Java](document-security-service-java-api.md#quick-start-soap-mode-removing-a-policy-from-a-pdf-document-using-the-java-api)

[Inicio rápido (modo SOAP): Revocación de un documento mediante la API de Java](document-security-service-java-api.md#quick-start-soap-mode-revoking-a-document-using-the-java-api)

[Inicio rápido (modo SOAP): Restablecimiento del acceso a un documento revocado mediante la API de Java](document-security-service-java-api.md#quick-start-soap-mode-reinstating-access-to-a-revoked-document-using-the-java-api)

[Inicio rápido (modo SOAP): Inspección de documentos PDF protegidos por políticas mediante la API de Java](document-security-service-java-api.md#quick-start-soap-mode-inspecting-policy-protected-pdf-documents-using-the-java-api)

[Inicio rápido (modo SOAP): Creación de una marca de agua mediante la API de Java](document-security-service-java-api.md#quick-start-soap-mode-creating-a-pdf-watermark-using-the-java-api)

[Inicio rápido (modo SOAP): Modificación de una marca de agua mediante la API de Java](document-security-service-java-api.md#quick-start-soap-mode-modifying-a-watermark-using-the-java-api)

[Inicio rápido (modo SOAP): Búsqueda de eventos mediante la API de Java](document-security-service-java-api.md#quick-start-soap-mode-searching-for-events-using-the-java-api)
...
El siguiente ejemplo de código Java crea una nueva directiva denominada *Permitir copia*. El conjunto de directivas al que se agrega la directiva se denomina *Conjunto de directivas globales*. Este conjunto de directivas existe de forma predeterminada. (Consulte [Creación de políticas](/help/forms/developing/protecting-documents-policies.md#creating-policies)).
...
## Inicio rápido (modo SOAP): Modificación de una directiva mediante la API de Java {#quick-start-soap-mode-modifying-a-policy-using-the-java-api}

El siguiente ejemplo de código Java modifica una directiva denominada *Permitir copia* estableciendo el período de concesión sin conexión en 40 días. (Consulte [Modificación de directivas](/help/forms/developing/protecting-documents-policies.md#modifying-policies)).
...
## Inicio rápido (modo SOAP): Eliminación de una directiva mediante la API de Java {#quick-start-soap-mode-deleting-a-policy-using-the-java-api}

El siguiente ejemplo de código Java elimina una directiva denominada *Permitir copia*. (Consulte [Eliminación de directivas](/help/forms/developing/protecting-documents-policies.md#deleting-policies).)
...
El siguiente ejemplo de código Java aplica una directiva denominada *Permitir copia* a un documento PDF denominado *Loan.pdf*. El conjunto de directivas al que se agrega la directiva se denomina *Conjunto de directivas globales*. El documento protegido por una política se guarda como archivo PDF denominado *PolicyProtectedLoanDoc.pdf. *(Consulte [Aplicación de políticas a Documentos PDF](/help/forms/developing/protecting-documents-policies.md#applying-policies-to-pdf-documents)).
...
## Inicio rápido (modo SOAP): Eliminación de una directiva de un documento PDF mediante la API de Java {#quick-start-soap-mode-removing-a-policy-from-a-pdf-document-using-the-java-api}

El siguiente ejemplo de código elimina una directiva de un documento PDF denominado *PolicyProtectedLoanDoc.pdf*. El documento PDF no seguro se guarda como *unProtectedLoan.pdf*. (Consulte [Eliminación de directivas de Documentos PDF](/help/forms/developing/protecting-documents-policies.md#removing-policies-from-pdf-documents)).
...
## Inicio rápido (modo SOAP): Revocación de un documento mediante la API de Java {#quick-start-soap-mode-revoking-a-document-using-the-java-api}

El siguiente ejemplo de código Java anula un documento protegido por una política denominado *PolicyProtectedLoanDoc.pdf*. Un documento PDF revisado se encuentra en la siguiente ubicación URL `https://[server]:[port]/RightsManagement/UpdatedLoan.pdf`. (Consulte [Revocar acceso a Documentos](/help/forms/developing/protecting-documents-policies.md#revoking-access-to-documents)).
...
## Inicio rápido (modo SOAP): Inspección de documentos PDF protegidos por políticas mediante la API de Java {#quick-start-soap-mode-inspecting-policy-protected-pdf-documents-using-the-java-api}

El siguiente ejemplo de código Java inspecciona un documento PDF protegido por una política denominado *PolicyProtectedLoanDoc.pd* f. (Consulte [Inspección de Documentos PDF protegidos por políticas](/help/forms/developing/protecting-documents-policies.md#inspecting-policy-protected-pdf-documents)).
...
## Inicio rápido (modo SOAP): Restablecimiento del acceso a un documento revocado mediante la API de Java {#quick-start-soap-mode-reinstating-access-to-a-revoked-document-using-the-java-api}

El siguiente ejemplo de código Java restablece el acceso a un documento PDF revocado denominado *PolicyProtectedLoanDoc.pdf*. (Consulte [Restablecimiento del acceso a Documentos revocados](/help/forms/developing/protecting-documents-policies.md#reinstating-access-to-revoked-documents)).
...
## Inicio rápido (modo SOAP): Creación de una marca de agua PDF mediante la API de Java {#quick-start-soap-mode-creating-a-pdf-watermark-using-the-java-api}

El siguiente ejemplo de código Java crea una nueva marca de agua PDF con el nombre &#39;Ejemplo de marca de agua PDF&#39;. Esta marca de agua contiene un solo elemento (consulte [Creación de marcas de agua](/help/forms/developing/protecting-documents-policies.md#creating-watermarks)).
...
## Inicio rápido (modo SOAP): Creación de una marca de agua de texto mediante la API de Java {#quick-start-soap-mode-creating-a-text-watermark-using-the-java-api}

El siguiente ejemplo de código Java crea una nueva marca de agua de texto con el nombre *Marca de agua de texto de muestra*. Esta marca de agua contiene un solo elemento.
...
## Inicio rápido (modo SOAP): Modificación de una marca de agua de texto mediante la API de Java {#quick-start-soap-mode-modifying-a-text-watermark-using-the-java-api}
...
## Inicio rápido (modo SOAP): Modificación de una marca de agua mediante la API de Java {#quick-start-soap-mode-modifying-a-watermark-using-the-java-api}

El siguiente ejemplo de código Java modifica una marca de agua con el nombre *Confidential* modificando el valor del atributo `opacity` a 80.
...
## Inicio rápido (SOAP): Aplicación de una política a un documento de Word mediante la API de Java {#quick-start-soap-applying-a-policy-to-a-word-document-using-the-java-api}

El siguiente ejemplo de código Java aplica una directiva denominada *Permitir copia* a un documento de Word denominado *Loan.doc*. El conjunto de directivas al que se agrega la directiva se denomina *Conjunto de directivas globales*. El documento protegido por una política se guarda como un archivo DOC denominado *PolicyProtectedLoanDoc.doc. *(Consulte [Aplicación de políticas a Documentos PDF](/help/forms/developing/protecting-documents-policies.md#applying-policies-to-pdf-documents)).
...
## Inicio rápido (modo SOAP): Eliminación de una directiva de un documento de Word mediante la API de Java {#quick-start-soap-mode-removing-a-policy-from-a-word-document-using-the-java-api}

El siguiente ejemplo de código elimina una directiva de un documento de Word denominado *PolicyProtectedLoanDoc.doc*. El documento de Word no seguro se guarda como *unProtectedLoan.doc*. (Consulte [Eliminación de directivas de Documentos de Word](/help/forms/developing/protecting-documents-policies.md#removing-policies-from-word-documents).)
...
## Inicio rápido (modo SOAP): Creación de una directiva abstracta mediante la API de Java {#quick-start-soap-mode-creating-an-abstract-policy-using-the-java-api}
...
## Inicio rápido (modo SOAP): Modificación de una directiva abstracta mediante la API de Java {#quick-start-soap-mode-modifying-an-abstract-policy-using-the-java-api}
...
## Inicio rápido (modo SOAP): Eliminación de una directiva abstracta mediante la API de Java {#quick-start-soap-mode-deleting-an-abstract-policy-using-the-java-api}
...
## Inicio rápido (modo SOAP): Protect muestra un PDF en el flujo de trabajo de instrucciones para un nuevo usuario, mediante la API de Java {#quick-start-soap-mode-protect-a-pdf-in-statement-workflow-for-a-new-user-using-the-java-api}
