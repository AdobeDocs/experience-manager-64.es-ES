...
seo-description: Invocar procesos creados en Workbench mediante solicitudes REST.
...
Al utilizar solicitudes REST, se recomienda no invocar los servicios de Forms directamente. En su lugar, invoque procesos creados en Workbench. Cuando cree un proceso diseñado para la invocación de REST, utilice un punto de inicio programático. En este caso, el extremo REST se agrega automáticamente. Para obtener información sobre la creación de procesos en Workbench, consulte [Uso de Workbench](https://www.adobe.com/go/learn_aemforms_workbench_63).

Cuando invoca un servicio mediante REST, se le solicita un nombre de usuario y una contraseña de los formularios AEM. Sin embargo, si no desea especificar un nombre de usuario y una contraseña, puede desactivar la seguridad del servicio.

Para invocar un servicio de Forms (un proceso se convierte en un servicio cuando se activa el proceso) mediante REST, configure un extremo REST. (Consulte &quot;Administración de extremos&quot; en [ayuda de administración](https://www.adobe.com/go/learn_aemforms_admin_63).)
...
El valor obligatorio `ServiceName` es el nombre del servicio de Forms que se va a invocar. El valor opcional `OperationName` es el nombre de la operación del servicio. Si no se especifica este valor, el nombre predeterminado es `invoke`, que es el nombre de la operación que inicio el proceso. El valor opcional `ServiceVersion` es la versión codificada en formato X.Y. Si no se especifica este valor, se utiliza la versión más reciente. El valor `enctype` también puede ser `application/x-www-form-urlencoded`.
...
* Objetos de colección como `java.util.List` y `java.util.Map`
   ...
   Si un servicio de Forms requiere un parámetro `com.adobe.idp.Document`, sólo se puede invocar el servicio mediante el método de POST HTTP. Si el servicio requiere un parámetro `com.adobe.idp.Document`, el cuerpo de la solicitud HTTP se convierte en el contenido del objeto de Documento de entrada.

   Si un servicio de AEM Forms requiere varios parámetros de entrada, el cuerpo de la solicitud HTTP debe ser un mensaje MIME de varias partes, tal como se define en RFC 1867. (RFC 1867 es un estándar que utilizan los navegadores web para cargar archivos en sitios web). Cada parámetro de entrada debe enviarse como parte independiente del mensaje de varias partes y codificarse en el formato `multipart/form-data`. El nombre de cada pieza debe coincidir con el nombre del parámetro.
   ...
   Si un parámetro de entrada es una lista, un cliente REST puede enviarlo especificando el parámetro varias veces (una para cada elemento de la lista). Por ejemplo, si A es una lista de documentos, la entrada debe ser un mensaje de varias partes compuesto por varias partes denominadas A. En este caso, cada pieza llamada A se convierte en un elemento de la lista de entrada. Si B es una lista de cadenas, la entrada puede ser un mensaje `application/x-www-form-urlencoded` compuesto por varios campos llamados B. En este caso, cada campo de formulario denominado B se convierte en un elemento de la lista de entrada.
   ...
   Si un mapa de entrada no es el parámetro de entrada solo de servicios, cada registro clave/valor que pertenece al mapa se puede enviar usando un parámetro denominado como concatenación del nombre del parámetro y la clave del registro. Por ejemplo, se puede enviar un mapa de entrada llamado `attributes` con una lista de los siguientes pares de clave/valores:
   ...
   Los parámetros de salida de los tipos de lista y asignación pasan a formar parte del mensaje XML resultante. La lista de salida se representa en XML como una serie de elementos XML con un elemento para cada elemento de la lista. Cada elemento recibe el mismo nombre que el parámetro de lista de salida. El valor de cada elemento XML es una de las dos cosas:

* Una representación textual del elemento en la lista (si la lista consta de tipos de cadena)
* Dirección URL que señala el contenido del Documento (si la lista está formada por objetos `com.adobe.idp.Document`)

   El siguiente ejemplo es un mensaje XML devuelto por un servicio que tiene un solo parámetro de salida denominado *lista*, que es una lista de enteros.
...
Un servicio de AEM Forms se puede invocar asincrónicamente mediante la sustitución `services` por `async_invoke` en la URL de invocación, como se muestra en el siguiente ejemplo.
...
El estado de la invocación asincrónica se puede recuperar usando una URL de invocación con `services` sustituida por `async_status`. La dirección URL debe contener un parámetro `job_id` que especifique el valor de identificador del trabajo asociado con esta invocación. Por ejemplo:
...
Una vez finalizado el trabajo y recuperado el resultado, el trabajo se puede eliminar usando una URL de invocación con `services` se sustituye por `async_dispose`. La dirección URL también debe contener un parámetro `job_id` que especifique el valor de identificador del trabajo. Por ejemplo:
...
## Sistema de informes de error {#error-reporting}

Si no se puede completar una solicitud de invocación sincrónica o asincrónica debido a una excepción que se está generando en el servidor, la excepción se notifica como parte del mensaje de respuesta HTTP. Si la URL de invocación (o la dirección URL `async_result` en caso de una invocación asincrónica) no tiene un sufijo .xml, el proveedor de REST devuelve el código HTTP `500 Internal Server Error` seguido de un mensaje de excepción.

Si la URL de invocación (o la dirección URL `async_result` en caso de una invocación asincrónica) tiene un sufijo .xml, el proveedor de REST devuelve el código HTTP `200 OK`seguido de un documento XML que describe la excepción en el siguiente formato.
...
Para proporcionar un transporte seguro a las invocaciones REST, un administrador de formularios AEM puede habilitar el protocolo HTTPS en el servidor de aplicaciones J2EE que aloja AEM Forms. Esta configuración es específica del servidor de aplicaciones J2EE; no forma parte de la configuración del servidor de formularios.
...
Para crear una directiva, debe definir tipos de datos complejos como un objeto `PolicyEntry`. Un objeto `PolicyEntry` define atributos como los permisos asociados con la directiva. (Consulte [Creación de políticas](/help/forms/developing/protecting-documents-policies.md#creating-policies)).
...
* Pasar valores de lista desglosada a un proceso de AEM Forms
...
El siguiente ejemplo HTML pasa dos valores `Boolean` a un proceso de AEM Forms denominado `RestTest2`. El nombre del método de invocación es `invoke` y la versión es 1.0. Tenga en cuenta que se utiliza el método Post HTML.
...
**Pasar valores de lista desglosada a un proceso**
