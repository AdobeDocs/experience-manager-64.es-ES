...
>[!NOTE]
>
>Se recomienda que no supere los 30 MB al rellenar un `BLOB` invocando su `setBinaryData` método. De lo contrario, existe la posibilidad de que `OutOfMemory`* se produce la excepción.*
...
1. Instale JDK 1.6 o posterior.
   ...
   * Añada el directorio JRE bin a la ruta de la clase. Este grupo se encuentra en el [*JDK_INSTALL_LOCATION*] directorio /jre.
...
Este proceso no se basa en un proceso de AEM Forms existente. Para seguir junto con los ejemplos de código, cree un proceso denominado `MyApplication/EncryptDocument`**uso de Workbench. (Consulte [Uso de Workbench](https://www.adobe.com/go/learn_aemforms_workbench_63).)
