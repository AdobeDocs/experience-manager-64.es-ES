...
Después de crear el cliente de servicio de ensamblador, hacer referencia al documento DDX y definir las opciones de tiempo de ejecución, puede invocar la operación para validar el documento DDX `invokeDDX` . Al validar el documento DDX, puede pasar `null` como parámetro de mapa (este parámetro generalmente almacena documentos PDF que el ensamblador necesita para realizar las operaciones especificadas en el documento DDX).
