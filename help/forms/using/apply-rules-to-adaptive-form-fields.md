...
El siguiente procedimiento muestra cómo crear una regla para mostrar un mensaje de error si el ID del cliente introducido en el formulario no existe en la base de datos. La regla también lleva se centra en y restablece el campo ID del cliente. La regla utiliza [la API dataIntegrationUtils del servicio del modelo de datos de formulario](/help/forms/using/invoke-form-data-model-services.md) para comprobar si el ID de cliente existe en la base de datos.

1. Pulse el campo **[!UICONTROL ID de cliente]** y pulse el icono `Edit Rules`. Se abrirá la ventana Editor de reglas.
1. Pulse el icono **[!UICONTROL + Crear]** para agregar una regla. Se abrirá el Editor visual.
