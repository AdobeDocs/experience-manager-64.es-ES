---
title: Endurecimiento y seguridad de los formularios AEM en un entorno OSGi
seo-title: Endurecimiento y seguridad de los formularios AEM en un entorno OSGi
description: Obtenga información sobre recomendaciones y prácticas recomendadas para proteger AEM Forms en el servidor OSGi.
seo-description: Obtenga información sobre recomendaciones y prácticas recomendadas para proteger AEM Forms en el servidor OSGi.
...
# Endurecimiento y seguridad de los formularios AEM en un entorno OSGi {#hardening-and-securing-aem-forms-on-osgi-environment}

Obtenga información sobre recomendaciones y prácticas recomendadas para proteger AEM Forms en el servidor OSGi.

La seguridad de un entorno de servidor es de suma importancia para una organización. En este artículo se describen recomendaciones y prácticas recomendadas para proteger los servidores que ejecutan AEM Forms. No se trata de un documento completo que endurezca el host para el sistema operativo. En su lugar, este artículo describe varios ajustes de seguridad que debe implementar para mejorar la seguridad de la aplicación implementada. Sin embargo, para garantizar que los servidores de aplicaciones se mantengan seguros, también debe implementar procedimientos de supervisión, detección y respuesta de seguridad, además de las recomendaciones que se proporcionan en este artículo. El documento también contiene las mejores prácticas y directrices para la seguridad de la información personal (información de identificación personal).

El artículo está dirigido a consultores, especialistas en seguridad, arquitectos de sistemas y profesionales de TI que son responsables de planificar el desarrollo de aplicaciones o infraestructuras y la implementación de AEM Forms. Estas funciones incluyen las siguientes funciones comunes:
...
La siguiente imagen muestra los componentes y protocolos que se utilizan en una implementación típica de AEM Forms, incluida la topología de cortafuegos adecuada:
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>
    <ul> 
     <li>/content/forms/formsets/files/</li> 
    </ul> </td> 
  </tr>
  <tr>
...
#### Configurar servidor de seguridad interno {#configure-internal-firewall}

Puede configurar el servidor de seguridad interno para permitir que determinados componentes de AEM Forms (por ejemplo, instancia de autor, instancia de procesamiento, bases de datos) se comuniquen con el conjunto de servidores de publicación y otros componentes internos mencionados en el diagrama de topología:
...
<table> 
 <tbody>
  ...
  <tr>
   <td>Servidor complementario de Forms Workflow (AEM Forms en el servidor JEE)</td> 
   <td>/soap/sdk</td> 
  </tr>
 </tbody>
</table>

#### Configurar los permisos del repositorio y las listas de control de acceso (ACL) {#setup-repository-permissions-and-access-control-lists-acls}

De forma predeterminada, los recursos disponibles en los nodos de publicación son accesibles para todos. El acceso de solo lectura está habilitado para todos los recursos. Se requiere para habilitar el acceso anónimo. Si tiene pensado restringir la vista de formulario y enviar el acceso solo a usuarios autenticados, utilice un grupo común para permitir que solo los usuarios autenticados tengan acceso de solo lectura a los recursos disponibles en los nodos de publicación. Las siguientes ubicaciones/directorios contienen recursos de formulario que requieren endurecimiento (acceso de solo lectura para usuarios autenticados):
...
Al configurar formularios para archivos adjuntos, verificar o previsualizar componentes, los datos correspondientes se almacenan en los nodos de publicación en /tmp/fd/. Los datos se purgan periódicamente. Puede modificar el trabajo de depuración de datos predeterminado para que sea más agresivo. Para modificar el trabajo programado para depurar datos, abra la consola web de AEM, abra la tarea de limpieza temporal de almacenamiento de AEM Forms y modifique la expresión de Cron.
...
Utilice las credenciales del servidor de procesamiento para el servicio de configuración de AEM DS para enviar datos desde el nodo de publicación al servidor de procesamiento. Se recomienda utilizar las credenciales de un usuario restringido no administrativo con acceso de lectura y escritura en el repositorio del servidor de procesamiento. Para obtener más información, consulte [Configuración de servicios de almacenamiento para borradores y envíos](/help/forms/using/configuring-draft-submission-storage.md).

### Datos seguros gestionados por el modelo de datos de formulario (FDM) {#secure-data-handled-by-form-data-model-fdm}

Utilice cuentas de usuario con privilegios mínimos requeridos para configurar orígenes de datos para el modelo de datos de formulario (FDM). El uso de una cuenta administrativa puede proporcionar acceso abierto de metadatos y entidades de esquema a usuarios no autorizados.\
...
* Solo los usuarios del grupo de usuarios de formularios pueden obtener una vista previa, crear borradores y enviar formularios.
...
* Hay un conjunto diferente de grupos predefinidos con privilegios específicos para cada persona. Asignar usuarios al grupo.
   ...
   * Un usuario del grupo de usuarios de formularios:
      ...
      * no se puede importar XDP ni ningún paquete que contenga XDP
   ...
   * Un usuario de plantillas-autores y de plantilla-poder-usuario puede obtener una vista previa y crear una plantilla.
...
* Agregue un usuario a un grupo de usuarios de flujo de trabajo para permitir que un usuario utilice aplicaciones de bandeja de entrada de AEM.

## Elementos de intranet seguros de un entorno de AEM Forms {#secure-intranet-elements-of-an-aem-forms-environment}

En general, los clústeres de procesamiento y el complemento Flujo de trabajo de formularios (AEM Forms en JEE) se ejecutan detrás de un servidor de seguridad. Así que se consideran seguros. Puede seguir realizando algunos pasos para endurecer estos entornos:

### Clúster de procesamiento seguro {#secure-processing-cluster}

Un clúster de procesamiento se ejecuta en el modo de creación, pero no se utiliza para actividades de desarrollo. No permita que un usuario normal se incluya en los grupos de autores de contenido y usuarios de formularios de un clúster de procesamiento.

### Utilice las prácticas recomendadas de AEM para proteger un entorno de AEM Forms {#use-aem-best-practices-to-secure-an-aem-forms-environment}

Este documento proporciona instrucciones específicas para el entorno de AEM Forms. Debe asegurarse de que la instalación de AEM subyacente es segura cuando se implementa. Para obtener instrucciones detalladas, consulte la documentación de la lista [de comprobación de seguridad de](/help/sites-administering/security-checklist.md) AEM.
