...
1. Arrastre el paso **[!UICONTROL Invocar flujo de trabajo DDX]** en la ficha Forms Workflow de la barra de tareas.
...
* **Crear mapa a partir de PayLoad**: Cuando se selecciona, todos los documentos de la carpeta de carga útil se agregan al mapa del Documento de entrada de la `invoke` API en el ensamblador. El nombre de nodo de cada documento se utiliza como clave en el mapa.
...
1. Arrastre el paso del flujo de trabajo **[!UICONTROL ConvertToPDFA]** bajo la ficha Forms Workflow de la barra de tareas.
...
El flujo de trabajo de procesamiento de formulario PDF es un envoltorio alrededor de la API de servicio de `renderPDFForm` Forms para crear un formulario PDF con una plantilla XDP y un xml de datos.
