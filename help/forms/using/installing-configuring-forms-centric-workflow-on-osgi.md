---
title: Instalación y configuración de un flujo de trabajo centrado en Forms en OSGi
seo-title: Instalación y configuración de un flujo de trabajo centrado en Forms en OSGi
description: 'Instale y configure AEM Forms Interactive Communications para crear correspondencia comercial, documentos, declaraciones, avisos de beneficios, correos electrónicos de marketing, facturas y kits de bienvenida. '
seo-description: 'Instale y configure AEM Forms Interactive Communications para crear correspondencia comercial, documentos, declaraciones, avisos de beneficios, correos electrónicos de marketing, facturas y kits de bienvenida. '
...
AEM Forms es una potente plataforma de clase empresarial. El flujo de trabajo centrado en formularios en OSGi es solo una de las capacidades de AEM Forms. Para obtener la lista completa de las funcionalidades, consulte [Introducción a AEM Forms](/help/forms/using/introduction-aem-forms.md).
...
La siguiente topología es una topología indicativa para ejecutar AEM Forms Interactive Communications, Correspondence Management, AEM Forms Data Capture y Forms-Centric workflow en funcionalidades OSGi. Para obtener información detallada sobre la topología, consulte [Arquitectura y topologías de implementación para AEM Forms](/help/forms/using/aem-forms-architecture-deployment.md).

![topología recomendada](assets/recommended-topology.png)

El flujo de trabajo centrado en AEM Forms en OSGi ejecuta la bandeja de entrada AEM y la IU de creación del modelo de flujo de trabajo AEM en las instancias de autor de AEM Forms.

## Requisitos del sistema {#system-requirements}

>[!NOTE]
>
>Vaya a la sección [Next step](#next-steps) del documento, si ya ha instalado AEM Forms en OSGi como se explica en el artículo [install and configure data capture features](/help/forms/using/installing-configuring-aem-forms-osgi.md) .
...
* La ruta de instalación de la instancia AEM no contiene espacios en blanco.
* Una instancia de AEM está funcionando. En la terminología de AEM, una &quot;instancia&quot; es una copia de AEM que se ejecuta en un servidor en modo de autor o publicación. Se necesita al menos una instancia de AEM (autor o procesamiento) para ejecutar el flujo de trabajo centrado en formularios en OSGi:

   * **Autor**: Instancia de AEM que se utiliza para crear, cargar y editar contenido y para administrar el sitio web. Una vez que el contenido está listo para su lanzamiento, se duplica en la instancia de publicación.
   * **Procesamiento:** una instancia de procesamiento es una instancia de  [autorización de AEM ](/help/forms/using/hardening-securing-aem-forms-environment.md) endurecida. Puede configurar una instancia de Autor y endurecerla después de realizar la instalación.
...
El paquete de complementos de AEM Forms es una aplicación implementada en AEM. El paquete contiene un flujo de trabajo centrado en Forms en OSGi y otras capacidades. Siga estos pasos para instalar el paquete de complementos:
...
1. Seleccione el paquete y haga clic en **[!UICONTROL Install]**.
   ...
   También puede descargar el paquete a través del enlace directo que aparece en el artículo [Versiones de AEM Forms](https://helpx.adobe.com/es/aem-forms/kb/aem-forms-releases.html).
...
1. Abra el archivo [AEM installation directory]\crx-quickstart\conf\sling.properties para editarlo.
   ...
   Si utilizó [AEM installation directory]\crx-quickstart\bin\start.bat para iniciar AEM, edite las propiedades de sling ubicadas en [AEM_root]\crx-quickstart\.
...
1. Agregue el paquete **sun.util.calendar** al campo **allowlist**. Haga clic en Guardar.
...
Dispatcher es una herramienta de almacenamiento en caché y equilibrio de carga para AEM. AEM Dispatcher también ayuda a proteger el servidor AEM de ataques. Puede aumentar la seguridad de su instancia de AEM utilizando Dispatcher junto con un servidor web de clase empresarial. Si utiliza [Dispatcher](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html), realice las siguientes configuraciones para AEM Forms:
...
1. Haga clic en **[!UICONTROL Configuración del canal web de formulario adaptable y comunicación interactiva]** para editar sus valores de configuración. En el cuadro de diálogo editar valores de configuración, especifique el número máximo de formularios o documentos que una instancia del servidor de AEM Forms puede almacenar en caché en el campo **Número de formularios adaptables**. El valor predeterminado es 100. Haga clic en **Guardar**.
   ...
   >[!NOTE]
   >
   >Para deshabilitar la caché, establezca el valor en el campo Número de formularios adaptables en **0**. La caché se restablece y todos los formularios y documentos se eliminan de la caché cuando se desactiva o cambia la configuración de la caché.
...
Ha configurado un entorno para utilizar el flujo de trabajo centrado en Forms en las capacidades OSGi. Ahora, los pasos para utilizar esta capacidad son:
