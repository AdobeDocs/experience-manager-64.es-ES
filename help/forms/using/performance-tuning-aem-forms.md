...
# Ajustar el rendimiento del servidor de AEM Forms {#performance-tuning-of-aem-forms-server}
...
Puede configurar y controlar la estrategia de almacenamiento en caché de AEM Forms mediante el componente **Configuraciones de Mobile Forms** en la consola de configuración web de AEM en:
...
* Linux: el servidor web Apache está preinstalado en el sistema.
...
1. Elimine los comentarios de las siguientes configuraciones de módulo en el archivo `APACHE_HOME/conf/httpd.conf`.
   ...
   >[!NOTE]
   >
   >En Linux, el `APACHE_HOME` es `/etc/httpd/` de forma predeterminada.
...
Es posible que experimente un rendimiento lento en los servidores que ejecutan un software antivirus. Un software de antivirus siempre activo (análisis en tiempo real) analiza todos los archivos de un sistema. Esto puede ralentizar el servidor y afectar al rendimiento de AEM Forms.
...
* El directorio temporal del servidor de aplicaciones. La ubicación predeterminada es:
   ...
   * (Jboss) [El directorio de instalación de AEM]\jboss\standalone\tmp
...
* **(Solo AEM Forms en JEE)** Los registros del servidor de AEM Forms y el directorio temporal. La ubicación predeterminada es:

   * Registros del servidor - `[AEM Forms installation directory]\Adobe\AEM forms\[app-server]\server\all\logs`
...
* Si el servidor AEM Forms experimenta un rendimiento lento incluso después de excluir los directorios sugeridos, excluya también el archivo ejecutable Java (java.exe).
>
