...
Algunas de las directrices para desarrollar los controladores de sincronización de contenido son las siguientes:
...
* El controlador solo debe informar de verdadero si actualiza la caché de ContentSync. Si se informa erróneamente de verdadero, AEM crear una actualización cuando no se produjo realmente una actualización.
* El controlador solo debe actualizar la caché si el contenido ha cambiado. No escriba en la caché si no es necesario un blanco. Esto hace que se cree una actualización innecesaria.

>[!NOTE]
>
>Habilitar *Registro de depuración de ContentSync* a través de las configuraciones del registrador OSGI en el paquete *com.day.cq.contentsync*. Esto permite rastrear qué controladores se ejecutaron y si actualizaron la caché y notificaron haber actualizado la caché.
...
* `type`: Nombre del tipo de configuración que se utiliza para procesar el contenido. Hay varios tipos disponibles y se describen en la sección *Tipos de configuración*.
...
Si la variable `cq:ContentSyncConfig` el nodo no `updateuser` propiedad, el valor predeterminado `anonymous` el usuario actualiza la caché.
...
**contenido** Representar contenido con [Procesamiento de solicitudes de Sling](/help/sites-developing/the-basics.md#sling-request-processing).
...
* [Creación AEM contenido para AEM Mobile On-demand Services](/help/mobile/mobile-apps-ondemand.md)
