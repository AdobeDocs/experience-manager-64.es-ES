...
El filtro PorterStem aplica el algoritmo de temporización Porter para inglés. Los resultados son similares a los del uso del temporizador de porter de bola de nieve con el argumento *language=&quot;English&quot;*. Pero este stemmer está codificado directamente en Java y no está basado en Snowball. No acepta una lista de palabras protegidas y sólo es apropiado para texto en inglés.
...
Para obtener información adicional sobre el número máximo de recursos que se pueden utilizar con AEM 6.4, consulte [Guía de tamaño de recursos](/help/assets/assets-sizing-guide.md).
...
Para configurar y configurar Asset Insights para Experience Manager implementado mediante Activación de Adobe (DTM), consulte [Configurar Asset Insights con AEM Assets](https://helpx.adobe.com/experience-manager/kt/assets/using/asset-insights-tutorial-setup.html).
...
Para obtener más información detallada, consulte [Guía de migración a CoralUI 3-based](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/legacy/coral2/migration.html).

### ¿Cómo personalizar el componente de búsqueda en AEM Assets? {#how-to-customize-the-search-component-in-aem-assets}

Para obtener más información sobre la ampliación/clasificación de búsqueda y la implementación adicional, consulte [Guía de implementación de búsqueda simple.](https://helpx.adobe.com/experience-manager/kt/sites/using/search-tutorial-develop.html)
