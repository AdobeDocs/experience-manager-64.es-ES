...
Estas directivas de CUG se implementan en una instancia de AEM a través de un módulo de autorización independiente llamado oak-authorized-cug. Este módulo viene con su propia gestión de controles de acceso y evaluación de permisos. En otras palabras, la configuración de AEM predeterminada incluye una configuración de repositorio de contenido Oak que combina varios mecanismos de autorización. Para obtener más información, consulte [esta página en la Documentación](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html)de Apache Oak.
...
Añadir el tipo de `granite:AuthenticationRequired` mezcla dentro de las rutas configuradas admitidas hará que el registro OSGi del controlador responsable se actualice con una nueva entrada adicional con la `sling.auth.requirements` propiedad. Si un requisito de autenticación determinado especifica la `granite:loginPath` propiedad opcional, el valor se registra además con el autenticador con un prefijo &#39;-&#39; para que se excluya del requisito de autenticación.
...
El destinatario de esta documentación es la evaluación de la ruta de inicio de sesión tal como la expone la `LoginPathProvider` interfaz interna. La implementación enviada desde AEM 6.3 se comporta de la siguiente manera:
...
* Cree un modelo de contenido del repositorio para que los requisitos de autenticación se apliquen a todo el árbol sin necesidad de volver a excluir los subárboles anidados del requisito.
...
Se necesitan los siguientes pasos para editar una directiva CUG existente. Tenga en cuenta que la política modificada debe escribirse de nuevo y que los cambios deben mantenerse utilizando `javax.jcr.Session.save()`.
...
La administración de controles de acceso JCR define un método de mejor esfuerzo para recuperar las directivas que surten efecto en una ruta determinada. Debido al hecho de que la evaluación de las directivas de CUG es condicional y depende de la configuración correspondiente para habilitarse, la llamada `getEffectivePolicies` es una manera conveniente de verificar si una determinada directiva de CUG está surtiendo efecto en una instalación determinada.

>[!NOTE]
>
>Tenga en cuenta la diferencia entre `getEffectivePolicies` y el siguiente ejemplo de código que sube por la jerarquía para averiguar si una ruta dada ya forma parte de un CUG existente.
...
>[!NOTE]
>
>Las modificaciones a un nodo de destinatario determinado mencionadas anteriormente solo se reflejarán en el autenticador de Apache Sling si el `RequirementHandler` se ha configurado y el destinatario está contenido en los árboles definidos por las rutas admitidas (consulte la sección Opciones de configuración).
>
...
A continuación se detallan los pasos para crear un nuevo requisito de autenticación. Tenga en cuenta que el requisito solo se registrará en Apache Sling Authenticator si `RequirementHandler` se ha configurado para el árbol que contiene el nodo destinatario.
...
A continuación se detallan los pasos para cambiar una ruta de inicio de sesión existente. La modificación solo se registrará en el autenticador de Apache Sling si `RequirementHandler` se ha configurado para el árbol que contiene el nodo destinatario. El valor de ruta de inicio de sesión anterior se eliminará del registro. Esta modificación no afecta al requisito de autenticación asociado al nodo de destinatario.
...
Pasos para eliminar un requisito de autenticación existente. El requisito solo se anulará del registro del autenticador de Apache Sling si el `RequirementHandler` árbol que contiene el nodo de destinatario está configurado.
...
También debe configurar el filtro [Remitente del reenvío](/help/sites-administering/security-checklist.md#the-sling-referrer-filter) Sling con todos los nombres de host que se puedan utilizar para acceder a AEM; por ejemplo, a través de CDN, Equilibrador de carga y otros.
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    ...
    </ul> <p>Consulte también Opciones <a href="/help/sites-administering/closed-user-groups.md#configuration-options" target="_blank">de configuración</a> a continuación.</p> </td> 
  </tr> 
  <tr> 
...
Para obtener más información, consulte la tabla de la sección Configuración [predeterminada desde AEM 6.3](#default-configuration-since-aem) a continuación.
...
Las piezas nuevas relacionadas con la autenticación se encuentran en el paquete del controlador de autenticación granite de **Adobe** ( `com.adobe.granite.auth.authhandler` versión 5.6.48). Este paquete forma parte de la instalación predeterminada AEM.

Para configurar la sustitución de requisitos de autenticación para la compatibilidad con CUG obsoleta, algunos componentes OSGi deben estar presentes y activos en una instalación AEM determinada. Para obtener más información, consulte **Características de los componentes** OSGi más adelante.
...
>[!NOTE]
>
>No hay configuración para **Apache Jackrabbit Oak CUG Excluir Lista** y **Adobe Granite Authentication Requirement y Login Path Handler** está presente en las instancias de creación predeterminadas.
...
|---|---|
| Rutas admitidas  `/content` | Los requisitos de autenticación definidos en el repositorio por medio del tipo de `granite:AuthenticationRequired` mezcla surtirán efecto a continuación `/content` sobre `Session.save()`. Se actualiza Sling Authenticator. Se omite Añadir el tipo de mezcla fuera de las rutas admitidas. |

## Desactivación de la autorización de CUG y del requisito de autenticación {#disabling-cug-authorization-and-authentication-requirement}
...
Para deshabilitar la compatibilidad con el requisito de autenticación proporcionado por el `granite.auth.authhandler` módulo, basta con quitar la configuración asociada con el requisito de autenticación de granito de **Adobe y el controlador** de ruta de inicio de sesión.
...
El mecanismo de importación de Apache Jackrabbit FileVault se ha ajustado para hacer frente a las políticas de control de acceso del tipo `PrincipalSetPolicy`.
...
### Replicación de Granito Adobe {#adobe-granite-replication}
...
### Controlador de autenticación Adobe Granite {#adobe-granite-authentication-handler}
...
La configuración de CUGs en combinación con LiveCopy se representa en el repositorio mediante la adición de un nodo adicional y una propiedad adicional de la siguiente manera:
...
### Diferencias en Configuración y Configuración de CUG {#diferences-in-cug-setup-and-configuration}

La compatibilidad **con el grupo cerrado de usuarios (CUG) de Granite** Adobe OSGi (CUG) ( `com.day.cq.auth.impl.cug.CugSupportImpl`) del componente obsoleto se ha sustituido por nuevos componentes para poder gestionar por separado las partes relacionadas con la autorización y la autenticación de la antigua funcionalidad CUG.
...
### Diferencias Relativas A La Autorización {#diferences-with-regards-to-authorization}

Las principales diferencias desde la perspectiva de la autorización se resumen en la lista siguiente:
...
Como se ha explicado anteriormente, las políticas de control de acceso de CUG ahora siempre se almacenan en el contenido, pero la evaluación de los permisos efectivos que resultan de esas políticas solo se aplicará si la evaluación de **CUG habilitada** está activada en la consola del sistema en la configuración de Apache Jackrabbit Oak **CUG.** De forma predeterminada, solo se habilita con el modo de ejecución &#39;publicar&#39;.
...
Para Añadir o eliminar un tipo de mezcla se requiere `jcr:nodeTypeManagement` otorgar privilegios. En la implementación anterior, el `jcr:modifyProperties` privilegio se utiliza para editar la propiedad residual.
...
#### Nodo Destinatario definido por tipo de mezcla {#target-node-defined-by-mixin-type}
