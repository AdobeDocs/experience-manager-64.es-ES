...
* Sin embargo, no afecta ni a los hermanos ni a los antepasados del nodo controlado de acceso;
...
El nuevo tipo de políticas de control de acceso para restringir el acceso de lectura para un CUG se administra usando la API de administración de control de acceso JCR y sigue los mecanismos descritos con la especificación [JCR 2.0](https://www.adobe.io/experience-manager/reference-materials/spec/jcr/2.0/16_Access_Control_Management.html).
...
Pasos para eliminar un requisito de autenticación existente. El requisito solo no se registrará en el autenticador de Apache Sling si el `RequirementHandler` se ha configurado para el árbol que contiene el nodo de destino.
...
>[!NOTE]
>
>Sin embargo, tenga en cuenta que al quitar la configuración no se desregistrará el tipo de mezcla, que aún se aplicaba a los nodos sin tener efecto.
