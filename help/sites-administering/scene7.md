...
>* Si utiliza Dynamic Media Classic con una configuración proxy personalizada, configure ambas configuraciones proxy del cliente HTTP, ya que algunas funcionalidades de Experience Manager utilizan las API 3.x y otras las API 4.x. 3.x está configurado con [http://localhost:4502/system/console/configMgr/com.day.commons.httpclient](http://localhost:4502/system/console/configMgr/com.day.commons.httpclient) y 4.x está configurado con [http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator](http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator).
>


## Integración Experience Manager/Dynamic Media Classic con Dynamic Media {#aem-scene-integration-versus-dynamic-media}

Los usuarios Experience Manager pueden elegir entre dos soluciones para trabajar con Dynamic Media:

* Uso de la instancia integrada de Experience Manager con *Dynamic Media Classic*.
* Uso *Dynamic Media* que se integra en Experience Manager.

Utilice los siguientes criterios para determinar qué solución es la mejor para usted:

* Si es **existente** Cliente de Dynamic Media Classic cuyos recursos de medios enriquecidos residen en Dynamic Media Classic para su publicación y envío, pero desea integrarlos con la creación de Sites (WCM) o Experience Manager Assets para su administración y, a continuación, utilizar la variable [Integración punto a punto Experience Manager/Dynamic Media Classic](#aem-scene-point-to-point-integration) descrito en este documento.
...
* **[!UICONTROL Medios interactivos]** - El **[!UICONTROL Medios interactivos]** es para aquellos titulares de carrusel, imágenes interactivas y vídeos interactivos que tienen interactividad en ellos, como zonas interactivas o mapas de imágenes. Este componente es inteligente. Según si agrega una imagen o un vídeo, tiene varias opciones. Además, el visor es interactivo. Es decir, el tamaño de la pantalla cambia automáticamente según el tamaño de la pantalla. Todos los visores son visores HTML5.
...
1. Siga las instrucciones específicas de su caso de uso:
   ...
   <table> 
    ...
    <td><p>Cargue recursos a Experience Manager y utilice Dynamic Media para generar representaciones para descargarlos o compartirlos. Publicar automáticamente Experience Manager Assets en Dynamic Media Classic para su envío.</p> <p><strong>Importante:</strong> Incurre en un procesamiento duplicado y las representaciones generadas en el Experience Manager no se sincronizan con Dynamic Media Classic</p> </td> 
    <td><p>Activado</p> <p>(Véase el paso 3)</p> </td> 
    <td>Desactivado</td> 
    <td>Desactivado</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">Activado</a></p> <p>(Véase el paso 4)</p> </td> 
...
### Carga de recursos desde Experience Manager Assets frente a Dynamic Media Classic {#uploading-assets-from-aem-assets-versus-from-scene}
...
Si los recursos ya están en Dynamic Media Classic y administrarlos en un repositorio común no es importante, exportarlos a Experience Manager Assets solamente para sincronizarlos de nuevo con Dynamic Media Classic para su envío es un viaje de ida y vuelta innecesario. De lo contrario, se prefiere mantener los recursos en un único repositorio y sincronizar con Dynamic Media Classic solo para su envío.
...
Para configurar Dynamic Media Classic para que se integre con Experience Manager, debe completar los siguientes pasos:
...
1. Si desea que los recursos del Experience Manager se carguen automáticamente en Dynamic Media Classic, active la opción y añada Dynamic Media Classic al flujo de trabajo de recursos de actualización de DAM. También puede cargar recursos manualmente.
1. Adición de componentes de Dynamic Media Classic a la barra de tareas. Al hacerlo, los usuarios pueden utilizar los componentes de Dynamic Media Classic en sus páginas de Experience Manager.
...
#### Carga en Dynamic Media Classic desde Experience Manager Assets {#uploading-to-scene-from-aem-assets}
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Los recursos que residen fuera de la carpeta de destino que se cargan manualmente en Dynamic Media Classic en WCM se colocan automáticamente en una carpeta ad hoc independiente en Dynamic Media Classic. Puede configurar esta funcionalidad en Cloud Configuration en Experience Manager.</td> 
  </tr> 
  <tr> 
   <td>Medios mixtos</td> 
  ...
  <tr> 
   ...
   <td><p>Si utiliza subcarpetas dentro de la carpeta de destino, utilice nombres únicos para cada recurso (independientemente de la ubicación) o configure Dynamic Media Classic (en el área Configuración) para que no sobrescriba los recursos independientemente de la ubicación.</p> <p>De lo contrario, se cargarán recursos con el mismo nombre que se carguen en una subcarpeta de destino de Dynamic Media Classic, pero se eliminará el recurso con el mismo nombre en la carpeta de destino. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
Si ejecuta un Experience Manager detrás de un proxy o tiene una configuración especial del cortafuegos, debe habilitar explícitamente los hosts de las diferentes regiones. Los servidores se administran en contenido en `/etc/cloudservices/scene7/endpoints` y se pueden personalizar según sea necesario. Pulse una dirección URL y, a continuación, edite para cambiar la dirección URL, si es necesario. En versiones anteriores de Experience Manager, estos valores estaban codificados.

Si va a `/etc/cloudservices/scene7/endpoints.html`, verá los servidores enumerados (y puede editarlos haciendo clic en la dirección URL):
...
>[!NOTE]
>
>Puede tener varias configuraciones: una configuración de nube representa a un usuario de una empresa de Dynamic Media Classic. Si desea acceder a otras empresas o usuarios de Dynamic Media Classic, cree varias configuraciones.

Para configurar el Experience Manager para que pueda publicar recursos en Dynamic Media Classic:

1. Pulse el icono del Experience Manager y vaya a **[!UICONTROL Implementación > Cloud Services]** para acceder a Adobe Dynamic Media Classic.

1. Toque **[!UICONTROL Configurar ahora]**.

   ![chlimage_1-297](assets/chlimage_1-297.png)

1. En el **[!UICONTROL Título]** y, opcionalmente, en la variable **[!UICONTROL Nombre]** , introduzca la información adecuada. Pulse **[!UICONTROL Crear]**.
...
1. Introduzca la dirección de correo electrónico, la contraseña y la región de su cuenta de Dynamic Media Classic y pulse **[!UICONTROL Conectarse a Dynamic Media Classic]**. Está conectado al servidor de Dynamic Media Classic y el cuadro de diálogo se amplía con más opciones.
...
1. Haga clic en **[!UICONTROL Aceptar]**. Experience Manager guarda la configuración.
...
### Activación del oyente de DAM de Adobe CQ Dynamic Media Classic {#enabling-the-adobe-cq-scene-dam-listener}
...
1. Toque . [!UICONTROL Herramientas] y, a continuación, vaya a **[!UICONTROL Operaciones > Consola web]**. Se abre la consola web.
1. Vaya a **[!UICONTROL Listener de Adobe CQ Dynamic Media Classic Dam]** y seleccione **[!UICONTROL Habilitado]** en el Navegador.
...
Cuando se configura una instancia de Experience Manager para gestionar la codificación de vídeo mediante Dynamic Media Classic, de forma predeterminada, cualquier trabajo de carga supera el tiempo de espera de 35 minutos. Para dar cabida a los trabajos de codificación de vídeo que pueden tardar más tiempo en ejecutarse, puede configurar esta configuración:
...
1. Cambie el número como desee en la variable **[!UICONTROL Tiempo de espera de trabajo activo]** campo . Se acepta cualquier número no negativo con la unidad de medida en segundos. De forma predeterminada, este número está establecido en 2100.
   ...
   >[!NOTE]
   >
   >Práctica recomendada: La mayoría de los recursos se introducen en cuestión de minutos como máximo (por ejemplo, imágenes). Sin embargo, en algunos casos, como los vídeos más grandes, el valor de tiempo de espera aumenta a 7200 segundos (dos horas) para dar cabida a un tiempo de procesamiento largo. De lo contrario, este trabajo de carga de Dynamic Media Classic se marca como **[!UICONTROL UploadFailed]** en los metadatos de JCR.

1. Pulse **[!UICONTROL Guardar]**.

### Carga automática desde Experience Manager Assets {#autouploading-from-aem-assets}

A partir de Experience Manager 6.3.2, Experience Manager Assets ya está configurado para usted, de modo que cualquier recurso digital que cargue en el administrador de recursos digitales se actualice automáticamente a Dynamic Media Classic si los recursos están en una carpeta de destino de CQ.
...
Para configurar la carga automática desde Experience Manager Assets:

1. Pulse el icono del Experience Manager y vaya a **[!UICONTROL Implementación > Cloud Services]** a continuación, en el encabezado Dynamic Media, en Configuraciones disponibles, pulse **[!UICONTROL dms7 (Dynamic Media]**)
1. Toque . **[!UICONTROL Avanzadas]** , seleccione **[!UICONTROL Habilitar carga automática]** casilla de verificación y, a continuación, pulse **[!UICONTROL OK]**. Ahora debe configurar el flujo de trabajo de DAM Asset para incluir la carga en Dynamic Media Classic.
...
1. Vuelva a la página de bienvenida del Experience Manager y pulse **[!UICONTROL Flujos de trabajo]**. Haga doble clic en el botón **Recurso de actualización DAM** flujo de trabajo para abrirlo.
1. En la barra de tareas, vaya a la **[!UICONTROL Flujo de trabajo]** componentes y seleccione **[!UICONTROL Dynamic Media Classic]**. Arrastrar **[!UICONTROL Dynamic Media Classic]** para acceder al flujo de trabajo y pulse **[!UICONTROL Guardar]**. Los recursos añadidos a Experience Manager Assets en la carpeta de destino se cargan automáticamente en Dynamic Media Classic.
...
Si desea publicar recursos inmediatamente al insertarlos en Dynamic Media Classic, no es necesario configurar ninguna opción. Este comportamiento es el predeterminado.
...
1. [Utilice el Admin Console para crear un caso de asistencia.](https://helpx.adobe.com/es/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html) En su caso de asistencia, solicite que la vista previa segura esté habilitada para su cuenta de Dynamic Media Classic.
1. Siga las instrucciones para [configure la vista previa segura para su cuenta de Dynamic Media Classic.](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish)
...
1. Pulse el icono del Experience Manager y vaya a **[!UICONTROL Implementación > Cloud Services]**, toque **[!UICONTROL Dynamic Media Classic]** y seleccione la configuración en Dynamic Media Classic.
1. Toque . **[!UICONTROL Avanzadas]** pestaña . En el **[!UICONTROL Habilitar Secure View]** menú desplegable, seleccione **[!UICONTROL Tras la activación de AEM Publish]** para insertar recursos en Dynamic Media Classic sin publicarlos. (De forma predeterminada, este valor se establece en **[!UICONTROL Inmediatamente]**, donde los recursos de Dynamic Media Classic se publican inmediatamente).

   Consulte [Documentación de Dynamic Media Classic](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish) para obtener más información sobre la prueba de recursos antes de hacerlos públicos.

   ![chlimage_1-302](assets/chlimage_1-302.png)

1. Pulse **[!UICONTROL Aceptar]**.

Habilitar la vista segura significa que los recursos se insertan en el servidor de vista previa seguro sin publicar.

Para ver si la vista previa segura está activada, vaya a un componente de Dynamic Media Classic en una página de Experience Manager. Pulse **[!UICONTROL Editar]**. El recurso tiene el servidor de vista previa seguro enumerado en la dirección URL. Después de publicar en el Experience Manager, el dominio del servidor en la referencia del archivo se actualiza de la dirección URL de vista previa a la URL de producción.
...
#### Activación de Dynamic Media Classic para WCM en la interfaz de usuario táctil {#enabling-scene-for-wcm-in-the-touch-optimized-user-interface}

Para habilitar Dynamic Media Classic para WCM en la IU táctil:

1. Pulse el icono del Experience Manager y vaya a **[!UICONTROL Sitios]** y luego la página raíz del sitio web (no específica del idioma).

1. En la barra de herramientas, seleccione [!UICONTROL configuración] icono y toque **[!UICONTROL Abrir propiedades]**.

1. Toque **[!UICONTROL Cloud Services]** y toque **[!UICONTROL Agregar configuración]** y seleccione **[!UICONTROL Dynamic Media Classic]**.
1. En el **[!UICONTROL Adobe Dynamic Media Classic]** lista desplegable, seleccione la configuración que desee y pulse **[!UICONTROL OK]**.
...
Para habilitar Dynamic Media Classic para WCM en la IU clásica:

1. En el Experience Manager, pulse **[!UICONTROL Sitios web]** y vaya a la página raíz del sitio web (no específica del idioma).

1. En la barra de tareas, toque la **[!UICONTROL Página]** icono y toque **[!UICONTROL Propiedades de página]**.
...
Puede configurar la carpeta a la que se cargan los recursos en Dynamic Media Classic cuando el recurso no esté en la carpeta de destino de CQ. Consulte Publicación de recursos desde fuera de la carpeta de destino de CQ.
...
1. Toque . **[!UICONTROL Avanzadas]** pestaña . En el **[!UICONTROL Carpeta ad hoc]** , puede modificar el **Ad Hoc** carpeta. De forma predeterminada, es la variable **name_of_the_company/CQ5_adhoc**.
...
## Habilitación de la compatibilidad con los parámetros de trabajo de carga de recursos/Dynamic Media Classic basados en tipos MIME {#enabling-mime-type-based-assets-scene-upload-job-parameter-support}
...
1. Pulse el icono del Experience Manager y vaya a **[!UICONTROL Herramientas > Operaciones > Consola web]**.
1. En el panel Configuración de la consola web de Adobe Experience Manager, en la **[!UICONTROL OSGi]** menú, toque **[!UICONTROL Configuración]**.
1. En la columna Nombre , busque y pulse **[!UICONTROL Servicio de tipo MIME de Adobe CQ Dynamic Media Classic Asset]** para editar la configuración.
1. En el área Asignación de tipo de MIME , pulse cualquier signo más (+) para agregar un tipo MIME.
...
1. En la esquina inferior derecha de la ventana de configuración, pulse **[!UICONTROL Guardar]**.
1. Vuelva al Experience Manager y, en el carril izquierdo, pulse CRXDE Lite.
...
1. Pulse el mimeType que acaba de añadir.

   Por ejemplo, `mimeTypes > application_postscript` O `mimeTypes > image_vnd.adobe.photoshop`.

1. En el lado derecho de la página CRXDE Lite, pulse el botón **[!UICONTROL Propiedades]** pestaña .
1. Especifique un parámetro de trabajo de carga de Dynamic Media Classic en la variable **[!UICONTROL jobParam]** campo de valor.
   ...
   >
   >Asegúrese de que el archivo PSD tiene &quot;capas&quot;. Si es estrictamente una imagen o una imagen con máscara, solo se procesa como imagen porque no hay capas para procesar.

1. En la esquina superior izquierda de la página CRXDE Lite, pulse **[!UICONTROL Guardar todo]**.
...
* Compruebe que el recurso que está intentando cargar se encuentra en la variable **[!UICONTROL Destino de CQ]** (especifica esta carpeta en la configuración de nube de Dynamic Media Classic).
* Si no es así, debe configurar la configuración de nube en **[!UICONTROL Propiedades de página]** para que esa página permita la carga en la **[!UICONTROL CQ adhoc]** carpeta.
