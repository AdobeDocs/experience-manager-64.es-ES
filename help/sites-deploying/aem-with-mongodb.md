...
Para obtener más información sobre MongoDB Cloud Manager, consulte la [Documentación de MongoDB](https://docs.mongodb.com/).
...
* `customBlobStore`
   ...
   Si la implementación almacena binarios en la base de datos, formarán parte del conjunto de trabajo. Por esta razón, se recomienda no almacenar binarios dentro de MongoDB, prefiriendo un almacén de datos alternativo como un `FileSystem` almacén de datos en un NAS.

* `cache`

   El tamaño de la caché en megabytes. Esto se distribuye entre las distintas cachés utilizadas en la variable `DocumentNodeStore`. El valor predeterminado es 256 MB. Sin embargo, el rendimiento de lectura de Oak se beneficiará de una caché más grande.
...
* `path`
   ...
   Ruta a la raíz del almacén de datos. Para una implementación MongoMK, debe ser un sistema de archivos compartido disponible para todas las instancias AEM. Normalmente se utiliza un servidor de Almacenamiento conectado en red (NAS). Para implementaciones en la nube como Amazon Web Service, la variable `S3DataFileStore` también está disponible.

* `cacheSizeInMB`

   El tamaño total de la caché binaria en megabytes. Se utiliza para almacenar en caché los binarios inferiores a la variable `maxCacheBinarySize` configuración.
...
Para obtener documentación sobre cómo configurar MongoDB con Amazon Web Service, consulte la siguiente guía [MongoDB en AWS](https://docs.aws.amazon.com/quickstart/latest/mongodb/welcome.html).
...
Marque la [Documentación de Dispatcher](https://helpx.adobe.com/es/experience-manager/dispatcher/using/dispatcher.html) para obtener más información sobre cómo configurarlo.
...
Si AEM se está ejecutando en una implementación del administrador de persistencia de MongoMK, [los nombres de página están limitados a 150 caracteres](/help/sites-authoring/managing-pages.md#page-name-restrictions-and-best-practices).
