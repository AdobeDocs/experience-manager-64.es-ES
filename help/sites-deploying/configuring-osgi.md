...
1. Se abrirá un cuadro de diálogo: Aquí puede editar según sea necesario; por ejemplo, establezca la variable **Nivel de registro** a `INFO`:
...
1. Abra CRXDE Lite.
1. En el **Herramientas** menú seleccionar **Consulta ...** .
...
Estas configuraciones se realizan creando `sling:OsgiConfig` nodos en el repositorio al que hace referencia el sistema. Estos nodos reflejan las configuraciones de OSGi y forman una interfaz de usuario para ellas. Para actualizar los datos de configuración, actualice las propiedades del nodo.
...
>[!NOTE]
>
>Lea también [cómo definir una restricción basada en repositorios solo para una instancia específica](https://helpx.adobe.com/experience-manager/kb/RunModeDependentConfigAndInstall.html).
...
1. Indica si una [modo de ejecución](/help/sites-deploying/configure-runmodes.md) es obligatorio. Cree la carpeta:
...
1. Si **Configuración** o **Configuración de fábrica** es necesario.
...
1. ¿Ya existe una configuración en `/libs`? Para enumerar todas las configuraciones en la instancia, utilice el **Consulta** herramienta en CRXDE Lite para enviar la siguiente consulta SQL:
...
1. Si aún no existe, cree la variable `config` carpeta ( `sling:Folder`):
...
Esto significa que una configuración genérica en `/libs` se puede ocultar mediante una configuración específica de proyecto en `/apps`.
...
1. Modificación de una configuración en `/libs` tendrá efecto inmediato, a menos que esté enmascarado por una configuración de `/apps`.
...
Por ejemplo, si se inició una instancia con los modos de ejecución `author,dev,emea`, nodos de configuración en `/apps/*/config.emea`, `/apps/*/config.author.dev/` y `/apps/*/config.author.emea.dev/` se aplicará, mientras que los nodos de configuración en `/apps/*/config.author.asean/` y `/config/author.dev.emea.noldap/` no se aplicará.
...
`com.day.cq.wcm.core.impl.VersionManagerImpl`, la configuración de `/apps/*/config.emea.author/` se aplicará.
...
* Configuración que cambia `admin` se guardan en `*.config` archivos en:
   ...
   * Este es el área de datos privada del administrador de configuración de OSGi y contiene todos los detalles de configuración especificados por `admin`, independientemente de cómo hayan entrado en el sistema.
...
>[!CAUTION]
>
>Debe ***never*** edite las carpetas o archivos en:
>
>`/crx-quickstart/launchpad/config`
