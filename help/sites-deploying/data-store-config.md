...
* `db`: Nombre de la base de datos de Mongo. El valor predeterminado es **Oak** . Sin embargo, las nuevas instalaciones AEM 6 utilizan **aem-author** como nombre predeterminado de la base de datos.
...
* `repository.home`: Ruta al inicio del repositorio donde se almacenan varios datos relacionados con el repositorio. De forma predeterminada, los archivos binarios se almacenan en `crx-quickstart/repository/datastore` directorio.

* `path`: Ruta al directorio bajo el cual se almacenarán los archivos. Si se especifica, tiene prioridad sobre `repository.home` valor.
...
1. Si ya AEM está configurado para trabajar con el almacenamiento Tar o MongoDB, elimine los archivos de configuración existentes del `aem-install/crx-quickstart/install` antes de continuar. Los archivos que deben eliminarse son:
...
* secretKey: La clave de acceso secreta de AWS. **Nota:** Cuando la variable `accessKey` o `secretKey` no se ha especificado, la variable [Función IAM](https://docs.aws.amazon.com/sdk-for-java/v1/developer-guide/java-dg-roles.html) se utiliza para la autenticación.
...
>[!NOTE]
>
>Al utilizar AEM 6.4 con TarMK, los binarios se almacenan de forma predeterminada en FileDataStore. Para usar TarMK con el DataStore de Azure, debe empezar a AEM usando la variable `crx3tar-nofds` runmode, por ejemplo:
