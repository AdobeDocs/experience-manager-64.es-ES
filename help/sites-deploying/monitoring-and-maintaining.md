...
1. Vaya a la consola **[Tools](/help/sites-administering/tools-consoles.md)**, seleccione **Versioning** y haga doble clic en **Purge Versions**.
...
* Las versiones de **Shirts** se purgan porque su edad de versión es buena a 2 días.
* Los **Tonga Fashions!** se depuran porque el número de versiones es bueno a 5.
...
>[!NOTE]
>
>Los registros de ImageServer y s7access no se incluyen en el paquete **Download Full** generado desde la página **system/console/status-Bundlelist**. Para fines de asistencia, si tiene problemas con Dynamic Media, anexe también los registros de ImageServer y s7access cuando se ponga en contacto con el Servicio de atención al cliente.
...
Para activar el nivel de registro de depuración para un Registrador, establezca la propiedad `org.apache.sling.commons.log.level` para depurar en el repositorio. Por ejemplo, en `/libs/sling/config/org.apache.sling.commons.log.LogManager` para configurar el [registro global de Apache Sling](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings).
...
1. Este paso solo es necesario cuando se requiere un nuevo Writer (es decir, con una configuración diferente a la predeterminada Writer).
   ...
   En `/apps/<project-name>/config`, cree un nodo para la nueva [Configuración del escritor de registro de Apache Sling](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings):
...
La Consola Felix también proporciona información sobre la compatibilidad con el registro Sling en `../system/console/slinglog`; por ejemplo `http://localhost:4502/system/console/slinglog`.
