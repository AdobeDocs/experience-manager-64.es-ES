...
* Utilice su instalación local (por ejemplo, utilice `http://localhost:4502/system/console/jcrresolver`) para determinar qué Resource Resolver está activo.
...
* **Modelo WCM** para definir el modo predeterminado.
...
* **URL para enviar** datos para configurar la URL utilizada para rastrear las estadísticas de la página (es vital si una solicitud de rastreador pasa por el despachante); por ejemplo, el valor predeterminado es  `http://localhost:4502/libs/wcm/stats/tracker`.
...
* **NIO para HTTP**, Indica si se utiliza o no NIO para HTTP. La opción predeterminada es true. Solo se utiliza si HTTP está habilitado.
