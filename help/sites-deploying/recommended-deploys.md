...
seo-description: En este artículo se describen las topologías recomendadas para AEM.
...
>[!NOTE]
>
>Esta página hace referencia a las topologías recomendadas para AEM. Para obtener más información sobre las capacidades de clustering y cómo configurarlas, consulte la documentación [de la API de](https://sling.apache.org/documentation/bundles/discovery-api-and-impl.html)Apache Sling Discovery.
...
* La conmutación por error no es automática. Debe detectarse externamente antes de que el sistema de conmutación por error pueda empezar a atender solicitudes.
...
>[!NOTE]
>
>La implementación en frío y en espera en este ejemplo de TarMK requiere que las instancias principal y en espera tengan una licencia independiente, ya que existe una replicación constante en el servidor de failover. Para obtener más información acerca de las licencias, consulte las Condiciones generales de licencia de [Adobe](https://www.adobe.com/legal/terms/enterprise-licensing.html).
...
Mantener los repositorios sincronizados se proporciona con el hecho de que el servidor de creación está publicando el mismo contenido para cada miembro del conjunto de servidores. For more information, see [Replication](/help/sites-deploying/replication.md).

En AEM Communities, el contenido generado por el usuario (UGC) nunca se replica. Para obtener información sobre la UGC en una granja TarMK, consulte [Consideraciones para comunidades](#considerations-for-aem-communities)AEM.
...
Este enfoque implica que varias instancias de Oak acceden a un conjunto de réplicas de MongoDB dentro de un solo centro de datos, lo que en realidad crea un clúster activo-activo para el entorno de creación de AEM. Los conjuntos de réplicas en MongoDB se utilizan para proporcionar alta disponibilidad y redundancia en caso de un fallo de hardware o de red.

![chlimage_1-18](assets/chlimage_1-18.png)

Las ventajas:

* Posibilidad de escalar horizontalmente con las nuevas instancias de creación de AEM
...
>[!NOTE]
>
>En el diagrama anterior, AEM Server 3 y AEM Server 4 se presentan con un estado inactivo, suponiendo una latencia de red entre los servidores AEM en el centro de datos 2 y el nodo principal MongoDB en el centro de datos 1 que es superior al requisito documentado [aquí](/help/sites-deploying/aem-with-mongodb.md#checklists). Si la latencia máxima es compatible con los requisitos, por ejemplo mediante el uso de zonas de disponibilidad, los servidores de AEM en el Centro de datos 2 también pueden estar activos, lo que crea un clúster de AEM activo-activo en varios centros de datos.

>[!NOTE]
>
>Para obtener información adicional sobre los conceptos arquitectónicos de MongoDB descritos en esta sección, consulte Replicación [de](https://docs.mongodb.org/manual/replication/)MongoDB.
...
Adobe recomienda encarecidamente que TarMK sea la tecnología de persistencia predeterminada utilizada por los clientes en todos los escenarios de implementación, tanto para las instancias de AEM Author como para las de Publish, excepto en los casos de uso descritos a continuación.

### Excepciones para elegir AEM MongoMK en lugar de TarMK en instancias de autor {#exceptions-for-choosing-aem-mongomk-over-tarmk-on-author-instances}
...
Es casi imposible predecir cuál será el modelo exacto de concurrencia después de que se active un nuevo sitio. Por lo tanto, Adobe recomienda tener en cuenta los siguientes criterios al evaluar si se utiliza MongoMK y dos o más nodos activos Author:
...
>[!NOTE]
>
>Se puede utilizar el Día duro para evaluar el rendimiento de la aplicación del cliente en el contexto de la configuración de hardware implementada. Encontrará más información sobre esta herramienta [aquí](/help/sites-developing/tough-day.md).
...
Hay un conjunto de requisitos previos y recomendaciones disponibles si está considerando implementar MongoMK para AEM:

**Requisitos previos obligatorios para implementaciones de MongoDB:**

1. El tamaño y la arquitectura de implementación de MongoDB deben formar parte de la implementación del proyecto con la ayuda de los arquitectos de Adobe Consulting o MongoDB que estén familiarizados con AEM;
...
1. Debe revisar el modelo de asistencia para implementaciones de AEM que incluyen MongoDB.

**Recomendaciones sólidas para implementaciones de MongoDB:**

* Consulte el [artículo](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager)MongoDB para Adobe Experience Manager;
...
>[!NOTE]
>
>Para todas las preguntas adicionales sobre estas directrices, requisitos previos y recomendaciones, póngase en contacto con el Servicio de atención al cliente de [Adobe](https://helpx.adobe.com/marketing-cloud/contact-support.html).

### Consideraciones para comunidades AEM {#considerations-for-aem-communities}

Para los sitios que planeen implementar comunidades [](/help/communities/overview.md)AEM, se recomienda [elegir una implementación](/help/communities/working-with-srp.md) optimizada para gestionar UGC anunciada por miembros de la comunidad desde el entorno de publicación.

Al utilizar un almacén [](/help/communities/working-with-srp.md)común, no es necesario replicar UGC entre el autor y otras instancias de publicación para obtener una vista coherente del UGC.
