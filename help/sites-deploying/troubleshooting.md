...
description: Este artículo trata algunos de los problemas de instalación que podría encontrar con AEM.
seo-description: Este artículo trata algunos de los problemas de instalación que podría encontrar con AEM.
...
Esta sección incluye información detallada sobre los registros disponibles para ayudarle a solucionar problemas y también incluye información sobre algunos de los problemas que podría encontrar con AEM.
...
AEM registra registros detallados que puede que desee configurar para solucionar problemas de instalación. Para obtener más información, consulte la sección [Trabajo con registros de auditoría y archivos](/help/sites-deploying/monitoring-and-maintaining.md#working-with-audit-records-and-log-files) de registro.

## Uso de la opción Verbose {#using-the-verbose-option}

Al iniciar AEM WCM, puede agregar la opción -v (detallado) a la línea de comandos como en: java -jar cq-wcm-quickstart-&lt;version>.jar -v.
...
### **Al hacer doble clic en el frasco de inicio rápido no se produce ningún efecto o se abre el archivo jar con otro programa (por ejemplo, el administrador de archivos){#double-clicking-the-quickstart-jar-does-not-have-any-effect-or-opens-the-jar-file-with-another-program-for-example-archive-manager}**
...
* Pruebe un menú contextual (generalmente con el botón secundario del mouse) en el inicio rápido de AEM WCM y seleccione &quot;Abrir con....&quot;
* Compruebe si Java o Sun Java están en la lista e intente ejecutar AEM WCM con él. Si tiene varias versiones de Java instaladas, seleccione la admitida.
   ...
   Si tiene éxito con este paso, y sus sistemas operativos ofrecen una opción para utilizar siempre el programa seleccionado para ejecutar los archivos .jar, selecciónelo. Hacer doble clic debería funcionar a partir de ahora.

* A veces, la reinstalación de la versión de Java admitida ayuda a restaurar la asociación correcta.
* Siempre puede ejecutar CRX mediante la línea de comandos o las secuencias de comandos start/stop como se describe anteriormente en este documento.
...
Especifique la opción de configuración de memoria al iniciar AEM WCM desde la línea de comandos. Los scripts de inicio/parada de AEM WCM o los scripts personalizados para administrar el inicio de AEM WCM también se pueden modificar para definir la configuración de memoria necesaria.
...
El síntoma habitual es que la ventana de inicio rápido de AEM WCM muestra &quot;AEM WCM en inicio, esperando el inicio del servidor....&quot; Si ese mensaje se muestra durante un tiempo relativamente largo, introduzca la URL de AEM WCM en la ventana del navegador de forma manual, utilizando el puerto predeterminado 4502 o el puerto en el que se está ejecutando la instancia: http://localhost:4502/.

Además, los registros pueden revelar el motivo por el que el explorador no se inicia.

En ocasiones, la ventana de inicio rápido de AEM WCM tiene el mensaje &quot;AEM WCM se ejecuta en http://localhost:port/&quot; y el navegador no se inicia automáticamente. En este caso, haga clic en la URL en la ventana de inicio rápido de AEM WCM (es un hipervínculo) o introduzca manualmente la URL en el navegador.
...
Consulte los detalles en los pasos de *Implementación de la aplicación* web AEM.

### **El tamaño del encabezado de respuesta puede ser mayor que 4 Kb{#response-header-size-can-be-greater-than-kb}**

Los errores 502 pueden indicar que el servidor web no puede gestionar el tamaño del encabezado de respuesta HTTP de AEM. AEM puede generar encabezados de respuesta HTTP que incluyan cookies de tamaño superior a 4 Kb. Asegúrese de que el contenedor de servlet esté configurado para que el tamaño máximo del encabezado de respuesta pueda superar los 4 kb.
...
Si instala o actualiza archivos JSP a Experience Manager en JBoss y no se compilan los servlets correspondientes, asegúrese de que el compilador JBoss JSP esté correctamente configurado. Para obtener más información, consulte la\
[Problemas de compilación JSP en el artículo JBoss](https://helpx.adobe.com/experience-manager/kb/jsps-dont-compile-jboss.html) .
