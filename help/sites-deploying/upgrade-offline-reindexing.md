...
¿Dónde `nodestore path` se encuentra el `mongo_ur` o `crx-quickstart/repository/segmentstore/`

Utilice el `--fake-ds-path=temp` parámetro en lugar de `–fds-path` para acelerar el proceso.
...
Dónde `datastore path` está la ruta al almacén de datos binario.

El almacén de texto creado se puede actualizar y reutilizar para volver a indexar escenarios en el futuro.

Para obtener más información sobre el proceso de extracción de texto, consulte la documentación [de ejecución de](https://jackrabbit.apache.org/oak/docs/query/pre-extract-text.html)Oak.
...
**1. Generar definiciones de índice Oak Lucene para la versión de AEM destinatario**
...
Para vaciar la definición de índice de la instancia de AEM de **origen** , ejecute este comando:
...
Dónde `datastore path` y `nodestore path` son de la instancia de AEM de **origen** .

A continuación, genere definiciones de índice a partir de la versión de **destinatario** AEM utilizando el paquete de repositorios Granite de la versión de destinatario.
...
>[!NOTE]
>
> El proceso de creación de definiciones de índice anterior solo se admite a partir de la `oak-run-1.12.0` versión anterior. La segmentación se realiza mediante el paquete de repositorios Granite `com.adobe.granite.repository-x.x.xx.jar`.
...
Cree un punto de comprobación en la instancia de AEM de **origen** de producción con una duración prolongada. Esto debe hacerse antes de clonar el repositorio.

A través de la consola JMX ubicada en `http://serveraddress:serverport/system/console/jmx`, vaya a `CheckpointMBean` y cree un punto de comprobación con una duración suficiente (por ejemplo, 200 días). Para ello, invoque `CheckpointMBean#createCheckpoint` with `17280000000` como argumento para la duración de duración en milisegundos.

Una vez finalizado, copie el ID de punto de comprobación recién creado y valide la duración mediante JMX `CheckpointMBean#listCheckpoints`.

>[!NOTE]
>
> Este punto de comprobación se eliminará cuando el índice se importe más tarde.

Para obtener más información, consulte la creación [de](https://jackrabbit.apache.org/oak/docs/query/oak-run-indexing.html#out-of-band-create-checkpoint) puntos de comprobación en la documentación de Oak.

**Realizar indexación sin conexión para las definiciones de índice generadas**

El reindexado de Lucene se puede realizar sin conexión mediante roble. Este proceso crea datos de índice en el disco debajo `indexing-result/indexes`. No **se escribe** en el repositorio y, por lo tanto, no es necesario detener la ejecución de la instancia de AEM. El almacén de texto creado se incluye en este proceso:
