...
La creación de un sitio para dispositivos móviles es similar a la creación de un sitio estándar, ya que también implica la creación de plantillas y componentes. Para obtener más información sobre la creación de plantillas y componentes, consulte las páginas siguientes: [Plantillas](/help/sites-developing/templates.md), [Componentes](/help/sites-developing/components.md) e [Introducción al desarrollo de AEM Sites](/help/sites-developing/getting-started.md). La principal diferencia consiste en habilitar las AEM funcionalidades móviles integradas dentro del sitio. Se logra creando una plantilla que depende del componente de página móvil.

También debe considerar la posibilidad de utilizar el diseño [](/help/sites-developing/responsive.md)interactivo, creando un único sitio que se adapte a varios tamaños de pantalla.
...
1. Cree el componente de página:
   ...
   * Establezca la `sling:resourceSuperType` propiedad en `wcm/mobile/components/page`
...
* Añade la ficha **Móvil** al cuadro de diálogo de propiedades de página.
...
Utilice Multi Site Manager (MSM) para crear una Live Copy móvil desde un sitio estándar. El sitio estándar se transforma automáticamente en un sitio móvil: el sitio de dispositivos móviles tiene todas las características de los sitios de dispositivos móviles (por ejemplo, la edición dentro de un emulador) y se puede administrar de forma sincronizada con el sitio estándar. Consulte la sección [Creación de una Live Copy para distintos Canales](/help/sites-administering/msm.md) en la página Administrador de varios sitios.

## API móvil de servidor {#server-side-mobile-api}

Los paquetes Java que contienen las clases móviles son:

* [com.day.cq.wcm.mobile.api](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/api/device/capability/package-summary.html) - define MobileConstances.

* [com.day.cq.wcm.mobile.api.device](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/api/device/package-summary.html) : define Device, DeviceGroup y DeviceGroupList.

* [com.day.cq.wcm.mobile.api.device.capabilities](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/api/device/capability/package-summary.html) : define DeviceCapability.
...
* [com.day.cq.wcm.mobile.core](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/core/package-summary.html) : define MobileUtil, que proporciona varios métodos de utilidad que giran en torno a WCM Mobile.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>mobileimage</td> 
   <td>Móvil</td> 
   <td>- basado en el componente<br /> de base de imagen: procesa una imagen si el dispositivo es capaz<br /> </td> 
  </tr> 
  <tr> 
   <td>mobilelist</td> 
   <td>Móvil</td> 
   <td>- basado en el componente<br /> de base de lista: listitem_teaser.jsp procesa una imagen si el dispositivo es capaz<br /> </td> 
  </tr> 
  <tr> 
   <td>mobilelogo</td> 
   <td>oculto</td> 
   <td>- basado en el componente<br /> de base del logotipo - representa una imagen si el dispositivo es capaz<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>Móvil</td> 
   <td>- basado en el componente<br /> de base textimage - procesa una imagen si el dispositivo es capaz</td> 
  </tr> 
  <tr> 
   <td>mobiletopnav</td> 
   <td>oculto</td> 
...
Para crear un emulador, consulte la sección [Creación de un emulador](/help/sites-developing/emulators.md) móvil personalizado en la página de emuladores genéricos.
...
Como los emuladores pueden asociarse a un grupo de dispositivos (consulte la sección [Creación de un grupo](#creating-a-device-group)de dispositivos), la asignación de un grupo de dispositivos a un sitio permite a los autores seleccionar entre los emuladores asociados al grupo de dispositivos para editar la página.

Para asignar un grupo de dispositivos a su sitio:

1. En el navegador, vaya a la consola **Siteadmin**.
1. Abra la página raíz del sitio de dispositivos móviles debajo de **Sitios** web.
...
Para obtener más información, consulte [Creación de Filtros](/help/sites-developing/groupfilters.md)de grupo de dispositivos.
...
1. Cree una nueva página debajo de **Herramientas** > **Móvil** > Grupos **** de dispositivos. En el cuadro de diálogo **Crear página** :
...
1. En CRXDE, agregue un archivo **static.css** que contenga los estilos del grupo de dispositivos debajo del `/etc/mobile/groups/special` nodo.

1. Abra la página Teléfonos **especiales** .
1. Para configurar el grupo de dispositivos, haga clic en el botón **Editar** situado junto a **Configuración**.
...
Para obtener más información, vaya a [Creación de Filtros](/help/sites-developing/groupfilters.md)de grupo de dispositivos.

### Uso de la base de datos WURFL™ {#using-the-wurfl-database}

AEM utiliza una versión truncada de la base de datos [WURFL](http://wurfl.sourceforge.net/index.php)™ para las funciones de dispositivos de consulta, como la resolución de pantalla o la compatibilidad con javascript, según el agente de usuario del dispositivo.

El código XML de la base de datos WURFL™ se representa como nodos a continuación `/var/mobile/devicespecs` analizando el `wurfl.xml`archivo en `/libs/wcm/mobile/devicespecs/wurfl.xml.` La expansión a nodos se produce la primera vez que se inicia el `cq-mobile-core` paquete.
...
* Utilice la versión disponible en AEM y configure un regexp que coincida con sus cadenas User-Agent y apunte a un dispositivo WURFL™ existente. Consulte [Añadir una coincidencia de agente-usuario basada en regexp](#adding-a-regexp-based-user-agent-matching).
...
AEM automáticamente analiza el `wurfl.xml` archivo y actualiza los nodos a continuación `/var/mobile/devicespecs`.
...
1. En el **CRXDE Lite**, cree un nodo debajo de /apps/wcm/mobile/devicespecs/regexp, por ejemplo: apple_ipad_ver1.
1. Añada las siguientes propiedades en el nodo:

   * **regexp**: expresión regular que define a los agentes de usuario, por ejemplo: .&amp;ast;Mozilla.&amp;ast;iPad.&amp;ast;AppleWebKit.&amp;ast;Safari.&amp;ast;
...
1. [Dirección URL específica del grupo de dispositivos](#defining-a-device-group-specific-url)
1. [URL basada en selector](#defining-selector-based-urls)

>[!NOTE]
>
>Para obtener más información sobre la integración de la biblioteca de clientes, consulte la sección [Uso de bibliotecas](/help/sites-developing/clientlibs.md) HTML del lado del cliente.
...
### Definición de una dirección URL específica de un grupo de dispositivos {#defining-a-device-group-specific-url}
...
>[!NOTE]
>
>Para obtener más información, lea la sección [Personalizado de BrowserMap](#using-browsermap-on-your-pages) .

### Definición de direcciones URL basadas en selector {#defining-selector-based-urls}

Si no se ha utilizado ninguno de los mecanismos anteriores para indicar un sitio alternativo para `BrowserMap`, entonces los selectores que utilizarán los nombres de los `DeviceGroups` se agregarán a los `URL`s, en cuyo caso debe proporcionar sus propios servlets que se encargarán de las solicitudes.
...
Para utilizar la biblioteca de cliente de BrowserMap estándar en una página, debe incluir el `/libs/wcm/core/browsermap/browsermap.jsp` archivo con una `cq:include`etiqueta en la sección de la `head` página.
...
Si desea personalizar `BrowserMap` la biblioteca `DeviceGroups` o agregar más sondeos, debe crear su propia biblioteca del lado del cliente en la que incruste la biblioteca del lado del `browsermap.standard`cliente.
...
1. Incluya el `broswermap.jsp` archivo en la sección del encabezado.
...
Puede forzar el dispositivo de cualquier solicitud para probar una versión específica de un sitio web agregando el `device` parámetro a la dirección URL. La siguiente URL mostrará la versión móvil del sitio web de Geometrixx Outdoors.
...
1. Un iPad envía una solicitud a la instancia de publicación de AEM, por ejemplo: `http://localhost:4503/content/geometrixx_mobile/en/products.html`
...
1. AEM asigna las funciones del dispositivo al grupo de dispositivos y se establece `touch` como selector de grupos de dispositivos.

1. AEM redirige la solicitud a `http://localhost:4503/content/geometrixx_mobile/en/products.touch.html.`
...
El usuario Alice es redireccionado a `coolpage.feature.html`y envía esa dirección URL a un amigo Bob que accede a ella con un cliente diferente que pertenece al grupo de `touch` dispositivos.
