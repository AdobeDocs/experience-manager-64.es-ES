...
Para obtener directrices de ajuste del rendimiento para flujos de trabajo DAM, consulte la Guía [de ajuste del rendimiento de](/help/assets/performance-tuning-guidelines.md)AEM Assets.
...
De forma predeterminada, la ventana **de mantenimiento** semanal tiene una tarea **de depuración** del flujo de trabajo, pero debe configurarse antes de que se ejecute. Para configurar las purgas de flujo de trabajo, se debe agregar una nueva configuración **de purga de flujo de trabajo de granito de** Adobe en la consola web.
...
* Los diseños de flujo de trabajo heredados (tanto en tiempo de diseño como en tiempo de ejecución) se mantienen en la siguiente ruta:
   ...
   >[!NOTE]
   >
   >Si estos diseños se editan *con la IU* AEM, los detalles se copiarán en las nuevas ubicaciones.
...
* Los iniciadores de flujo de trabajo heredados se mantienen en la siguiente ruta:
   ...
   >[!NOTE]
   >
   >Si estas definiciones se editan *con la IU* AEM, los detalles se copiarán en las nuevas ubicaciones.
...
* `Session.Save` no se debe llamar desde dentro de un paso de flujo de trabajo:
   ...
   * se recomienda adaptar la sesión jcr del flujo de trabajo; no `save` es necesario, ya que el motor de flujos de trabajo guarda la sesión automáticamente una vez que el flujo de trabajo ha terminado de ejecutarse.
